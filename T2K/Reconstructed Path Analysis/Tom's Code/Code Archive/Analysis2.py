...
class Analysis:
	...
	def beforeLoop(self):
		...
		self.loadInputFiles()
		self.loadOutputFile()
		self.addHistogram3D("XYZ_Position", "Position of CCQE interaction in FGDs", 5, -850, 850, 5, -800, 900, 5, 100, 2000)#Histogram of CCQE neutrino interaction location	
		...
		for ivtx in xrange(NVtx):
			...
			if(EvtCodeSplitList[2][:2]=="N:"):
				...
				if(EvtCodeSplitList[3][:2]=="q:"):
					...
					InteractionType=EvtCodeSplitList[3].split(",")[1]#could just say coherent?					
				...
				InteractionType=EvtCodeSplitList[2].split(",")[1]#could just say coherent?
			...
			if(inFGD and Current=="CC" and InteractionType=="QES"):
