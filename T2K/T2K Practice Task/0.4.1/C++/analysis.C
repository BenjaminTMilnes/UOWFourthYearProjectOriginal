...
void analysis(int NEvents ){
	...
	TH1* DeltaMomentumHistogram = new TH1F("MomDelta","Histogram of delta baryon momentum",100,0,5);
	TH1* ProtonPionMomentumHistogramP = new TH1F("MomPPiP","Histogram of proton momentum from Delta -> p + pi0 decay",100,0,4);
	TH1* ProtonPionMomentumHistogramPI = new TH1F("MomPPiPi","Histogram of pion momentum from Delta -> p + pi0 decay",100,0,4);
...
//                 cout << "Energy of incoming neutrino: " << vtx->StdHepP4[0][3] << " GeV" << endl;
				...
				NParticles=vtx->StdHepN;
					...
					for (int k=0;k<NParticles;k++) {//Iterate over all particles in the event
						if((vtx->StdHepPdg[k]==2214)&&(vtx->StdHepStatus[k]!=0)) {
							...
							if((ProtonPresent==1)&&(PhotonPresent==1)) {//check if proton + pion channel
								PhotonProtonCount++;
							}
	...
	xAxis = InteractionModeHistogram->GetXaxis();
	...
	yAxis = InteractionModeHistogram->GetYaxis();
	yAxis->SetTitle("Number");
	yAxis->CenterTitle();
	yAxis->SetTitleOffset( 1.2 );
	...
	xAxis = vertexXYHist->GetXaxis();
	xAxis->SetTitle("x axis");
	xAxis->CenterTitle();
//	xAxis->SetTitleOffset( 1.2 );


	yAxis = vertexXYHist->GetYaxis();
...
//	yAxis->SetTitleOffset( 1.2 );
	...
	xAxis = vertexYZHist->GetXaxis();
	xAxis->SetTitle("y axis");
	xAxis->CenterTitle();
//	xAxis->SetTitleOffset( 1.2 );


	yAxis = vertexYZHist->GetYaxis();
...
//	yAxis->SetTitleOffset( 1.2 );
	...
	xAxis = DeltaMomentumHistogram->GetXaxis();
	xAxis->SetTitle("Momentum (GeV/c)");
	xAxis->CenterTitle();
//	xAxis->SetTitleOffset( 1.2 );


	yAxis = DeltaMomentumHistogram->GetYaxis();
	yAxis->SetTitle("Number");
	yAxis->CenterTitle();
//	yAxis->SetTitleOffset( 1.2 );
	...
	xAxis = ProtonPionMomentumHistogramP->GetXaxis();
	xAxis->SetTitle("Momentum (GeV/c)");
	xAxis->CenterTitle();
//	xAxis->SetTitleOffset( 1.2 );


	yAxis = ProtonPionMomentumHistogramP->GetYaxis();
	yAxis->SetTitle("Number");
	yAxis->CenterTitle();
//	yAxis->SetTitleOffset( 1.2 );
	...
	xAxis = ProtonPionMomentumHistogramPI->GetXaxis();
	xAxis->SetTitle("Momentum (GeV/c)");
	xAxis->CenterTitle();
//	xAxis->SetTitleOffset( 1.2 );


	yAxis = ProtonPionMomentumHistogramPI->GetYaxis();
	yAxis->SetTitle("Number");
	yAxis->CenterTitle();
//	yAxis->SetTitleOffset( 1.2 );
