	...
	def runEvent(self):
		...
		for PIDCounter in xrange(NPIDs):#Loop over the PIDs if they exist
			...
			if(len(PID.ParticleIds)>0):	#The reconstructed PID is put into a vector of possible PDGs: The best fitting PDG is given first and then the whole list (including the best
				...
				if(PID.ParticleIds[0]!=0 and PID.PIDWeights[0]>=0 and SuitableTPCNodeNumber):	#PDG is only nonzero if the event was in the inner, particle sensitive part of detector (eg not ECal)
				...
				if(ParticleId!=None):
					...
					if(ParticleId==2212): PIDDetectorListProtonIndex=PIDCounter
		...
		for PIDObjectListCounter in xrange(len(PIDObjectList)):
			...
			if(PIDObjectList[PIDObjectListCounter].ReconParticleID==2212):#Looking for reconstructed proton track
