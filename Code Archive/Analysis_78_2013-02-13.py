...
class Analysis:
		...
		for i in range(EventNumber):
			...
			if (This.InputTimeTest == False):
				...
				if ((OutputData or (i == EventNumber - 1)) and (SuitableEvents > 0)):
					...
					#This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents13EventsRemaining"].Title = "Selection 13 - The smallest photon angle was less than " + str(This.Parameters["PhotonAngularLocalityLimit"]) + " degees - Events Remaining"
					...
					#This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents13Delta"].Title = "Selection 13 - The smallest photon angle was less than " + str(This.Parameters["PhotonAngularLocalityLimit"]) + " degees - Delta -> p gamma Events Remaining"
	...
	def InitialiseHistograms(This):
		...
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_SeparationX", "Photons/ElectronsAndAntielectrons/", "Electron positron separation in X axis", "Separation (mm)", 100, 0, This.Parameters["LocalityLimit"] * 10, "Number")
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_SeparationY", "Photons/ElectronsAndAntielectrons/", "Electron positron separation in Y axis", "Separation (mm)", 100, 0, This.Parameters["LocalityLimit"] * 10, "Number")
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_SeparationZ", "Photons/ElectronsAndAntielectrons/", "Electron positron separation in Z axis", "Separation (mm)", 100, 0, This.Parameters["LocalityLimit"] * 10, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_SeparationModulus", "Photons/ElectronsAndAntielectrons/", "Separation modulus between electrons and antielectrons", "Separation (mm)", 100, 0, This.Parameters["LocalityLimit"] * 10, "Number")
		...
		This.HistogramCollection1.NewStackedHistogram1D("Proton_Muon_Separation", "MuLeptons/", "Separation between the single proton and single mu lepton in suitable events", "Separation (cm)", 100, 0, int(This.Parameters["LocalityLimit"] * 5), "Number")	
		...
		This.HistogramCollection1.NewHistogram1D("MultipleProtonECalMultiplicityECal", "Photons/Recon/Multiplicity/", "ECal multiplicity for events that have a proton and photon within angle and synchronicity", "Multiplicity", 10, 0, 10, "Number")
		...
		This.HistogramCollection1.NewHistogram1D("MultipleProtonECalMultiplicityPP", "Photons/Recon/Multiplicity/", "ECal multiplicity for events that have a proton and photon within angle and synchronicity", "Multiplicity", 10, 0, 10, "Number")
	...
	def runEvent(This):
		...
		DeltaPGammaCriteria = [True, False, False, False, False, False, False, False, False, False, False, False, False, False]
		...
		if ((ProtonsInFGD1 or ProtonsInFGD2) and DeltaPGammaCriteria[4]):
			DeltaPGammaCriteria[5] = True
				
		if (NumberOfReconstructedMuLeptonTracks == 0 and DeltaPGammaCriteria[5]):
			DeltaPGammaCriteria[6] = True
			
		if (NumberOfReconstructedElectronTracks == 0 and DeltaPGammaCriteria[6]):
			DeltaPGammaCriteria[7] = True	
		
		if (NumberOfReconstructedAntiElectronTracks == 0 and DeltaPGammaCriteria[7]):
			DeltaPGammaCriteria[8] = True
		
		if ((NTrackerECalRecon > 0) and DeltaPGammaCriteria[8]):
			DeltaPGammaCriteria[9] = True
		
		if ((NumberOfProtonsAndPhotonsWithinSynchronicity > 0) and DeltaPGammaCriteria[9]):
			DeltaPGammaCriteria[10] = True
			
		if ((NumberOfProtonsAndPhotonsWithinAngle > 0) and DeltaPGammaCriteria[10]):
			DeltaPGammaCriteria[11] = True
			
		if (NumberOfReconstructedProtonTracks <= 3 and DeltaPGammaCriteria[11]):
			DeltaPGammaCriteria[12] = True
			
		if (NTrackerECalRecon <= 3 and DeltaPGammaCriteria[12]):
			DeltaPGammaCriteria[13] = True
		...
		ProtonPhotonToElectrons = [True, False, False, False, False, False, False, False, False, False, False, False, False, False]
		...
		if ((ProtonsInFGD1 or ProtonsInFGD2) and ProtonPhotonToElectrons[4]):
			ProtonPhotonToElectrons[5] = True
				
		if (NumberOfReconstructedMuLeptonTracks == 0 and ProtonPhotonToElectrons[5]):
			ProtonPhotonToElectrons[6] = True
			
		if (NumberOfReconstructedElectronTracks == 0 and ProtonPhotonToElectrons[6]):
			ProtonPhotonToElectrons[7] = True	
		
		if (NumberOfReconstructedAntiElectronTracks == 0 and ProtonPhotonToElectrons[7]):
		...
		if (EventContainsElectronPositronPair and ProtonPhotonToElectrons[8]):
			ProtonPhotonToElectrons[9] = True
			
		if (ElectronPositronPairPointsWithinAngle >= 1 and ProtonPhotonToElectrons[9]):
			ProtonPhotonToElectrons[10] = True
		
		if (ElectronPositronPairPointsWithinTime >= 1 and ProtonPhotonToElectrons[10]):
			ProtonPhotonToElectrons[11] = True
			
		if (NumberOfReconstructedProtonTracks <= 3 and ProtonPhotonToElectrons[11]):
			ProtonPhotonToElectrons[12] = True
			
		if (NTrackerECalRecon <= 4 and ProtonPhotonToElectrons[12]):
			ProtonPhotonToElectrons[13] = True
	...
	def EventContainsElectronPositron(This, PIDParticles1, Protons, TruthInteractionType):
		...
		for Electron1 in Electrons:
			for Positron1 in Positrons:
				...
				if (Electron1.CorrectlyReconstructed() == True and Positron1.CorrectlyReconstructed() == True):
					This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_SeparationModulus"].Populate("0", math.fabs(Separation.Modulus()), "Correctly Reconstructed Electron-Antielectrons")
				elif (Electron1.CorrectlyReconstructed() == True and Positron1.CorrectlyReconstructed() == False):
					This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_SeparationModulus"].Populate("1", math.fabs(Separation.Modulus()), "Correctly Reconstructed Electron")
				elif (Electron1.CorrectlyReconstructed() == False and Positron1.CorrectlyReconstructed() == True):
					This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_SeparationModulus"].Populate("2", math.fabs(Separation.Modulus()), "Correctly Reconstructed Antielectron")
				else:
					This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_SeparationModulus"].Populate("3", math.fabs(Separation.Modulus()), "Incorrectly Reconstructed Electron-Antielectrons")
