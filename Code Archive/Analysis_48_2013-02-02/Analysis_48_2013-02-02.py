...
class Analysis:
		...
		for CutCounter in xrange(DeltaPGammaCutNumber):
			if (DeltaPGammaCriteria[CutCounter]):
				...
				if (CutCounter < DeltaPGammaProtonSelectionCut):
					...
					#print SelectedProton.Process, TrueEventContainsDelta1ToProtonPhoton
	...
	def MuonProtonSameVertex(This, PIDParticles1, Proton):
		
		MuonCount = 0
		MuonProtonSameVertex = False
		
		for Particle in PIDParticles1:
			
			if (Particle.ReconParticleID == ParticleCodes.MuLepton):
				
				MuonCount += 1
				
				MuonFrontPosition = Geometry.ThreeVector(Particle.ReconFrontPositionX, Particle.ReconFrontPositionY, Particle.ReconFrontPositionZ)
				ProtonFrontPosition = Geometry.ThreeVector(Proton.ReconFrontPositionX, Proton.ReconFrontPositionY, Proton.ReconFrontPositionZ)
				
		if (MuonCount == 1):
			
			FrontPositionSeperation = ProtonFrontPosition - MuonFrontPosition
			
			if (FrontPositionSeperation.Modulus() < NumericalCuts.MuonProtonSeparation):
