...
class HistogramCollection: 
	...
	def ToHistogramStorage(This): # This is transitional code and will be deleted later.
		...
		for HistogramKey1, Histogram1 in This.Histograms.items():
...
class Histogram1D(Histogram):
	...
	def ToROOTHistogram(This):
		...
		TH1D1.SetLineColor(Colours.Black)
			...
			for DataGroupReference1 in This.DataGroups.keys():
				if ((len(This.DataGroups[DataGroupReference1].Data) > 0) or (DisplayStackedHistogramGroupsWithNoData == True)):
					...
					if (AutoColour == True):
						...
						This.TH1Ds[DataGroupReference1].SetLineColor(Colours.Black)
					...
					if (This.DataGroups[DataGroupReference1].LegendTitle == None):
						This.DataGroups[DataGroupReference1].LegendTitle = ""
...
def ToLocator(Text):
	...
	Locator = re.sub(r"[\-]", "_HyphenOrMinus_", Locator)
