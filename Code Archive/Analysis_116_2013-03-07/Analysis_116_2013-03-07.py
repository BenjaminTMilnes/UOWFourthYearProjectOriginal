...
class Analysis:
	...
	def __init__(This, InputFileLocatorList, OutputROOTFileLocator, SimulationInformationFileLocator, SimulationInformation):
		...
		This.LastFileRead = ""
		This.TotalEntries = 0
		This.EventsInLastFile = 0
		
		This.BasicInformation = This.LoadModule("HeaderDir/BasicHeader")
		This.BasicDataQuality = This.LoadModule("HeaderDir/BasicDataQuality")
		This.BeamSummaryData = This.LoadModule("HeaderDir/BeamSummaryData")
		
		This.Truth_GRTVertices = None
		This.Truth_Vertices = None
		This.Truth_Trajectories = None
		...
		This.MonteCarloData = False
		
		This.TrueEventInteraction = None
				
		This.EventCollection1 = Simulation.EventCollection()
		
		This.HistogramCollection1 = Charts.HistogramCollection()
		
		This.ProtonsOnTargetCounterData = 0
		This.ProtonsOnTargetCounterMC = 0
		...
		This.Parameters["LocalityLimit"].Z = 20 # !!! Note - I lowered this to 20 mm from 50 mm so that it looks like we learnt something from how the z axis distribution is less well spread.
		...
		This.Parameters["PIDTrackMultiplicityLimit"] = 3
		This.Parameters["ECalMultiplicityLimit"] = 2
		
		#This.Parameters["PathMultiplicityLimit"] = 3
		
		This.Parameters["MuonMultiplicityCut"] = 0
		This.Parameters["ElectronMultiplicityCut"] = 0
		This.Parameters["PositronMultiplicityCut"] = 0
	...
	def LoadInputFiles(This, DesiredEventNumber):		
		...
		for FileLocator in This.InputFileLocatorList:
			...
			if (This.Modules[0].GetEntries() <= DesiredEventNumber):
				...
				RealDataSpillFile = False
			
				if (This.MonteCarloData == False):
					
					FoldersAndFiles = FileLocator.split("/")

					File = FoldersAndFiles[len(FoldersAndFiles) - 1]
					
					if (len(File.split("_")) >=3):
						
						Trigger = File.split("_")[2]
					...
					if (Trigger != "cos"):
						RealDataSpillFile = True
					
			
				if ((RealDataSpillFile == True) or (This.MonteCarloData == True)):
					...
					This.LastFileRead = FileLocator
					
					CurrentEventNumber = This.Modules[0].GetEntries()
					This.EventsInLastFile = CurrentEventNumber - PreviousEventNumber
		...
		This.TotalEntries = This.Modules[0].GetEntries()
	...
	def Analyse(This, DesiredEventNumber = 999999999):
		...
		for i in range(EventNumber):
			...
			try:
				...
				if (This.IsValidSpill()):
					...
					try:
						...
						if (This.MonteCarloData):
							...
							if (ValidSpillNumber == 1):
								
								PreviousFile = This.Modules[0].GetFile()

							CurrentFile = This.Modules[0].GetFile()

							if ((CurrentFile != PreviousFile) or (ValidSpillNumber == 1)):
								
								if (CurrentFile.GetName() != This.LastFileRead):
								
									This.ProtonsOnTargetCounterMC += This.ProtonsOnTargetMC()
									
								elif (CurrentFile.GetName() == This.LastFileRead):
									
									EventsUsedInLastFile = This.EventsInLastFile - (This.TotalEntries - i)

									ProportionOfLastFile = float(EventsUsedInLastFile) / float(This.EventsInLastFile)
									
									This.ProtonsOnTargetCounterMC += (This.ProtonsOnTargetMC() * ProportionOfLastFile)

							PreviousFile = CurrentFile

						else:
							This.ProtonsOnTargetCounterData += This.ProtonsOnTargetData()
						...
						print "Warning: Could not count the protons on target for event" , i
						print "The following error occured:" , traceback.format_exc()
					...
					if (This.InputTimeTest == False):
						...
						try:
							...
							if (This.Reconstructed_Global.NPIDs > 0):
								This.ReconstructedStatistics.Statistics["NPIDEventsFullMC"].Quantity += 1
							...
							if (SuitableTPCNodeNumber == True):
								This.ReconstructedStatistics.Statistics["NPIDEventsFullMCTPC"].Quantity += 1
						
						except KeyboardInterrupt:
							raise
									
						except:
							print "Warning: An error occured when analysing the PIDs for event number" , i
							print "The following error occured:" , traceback.format_exc()
							...
							try:
								...
								for GRTVertex1 in This.Truth_GRTVertices.Vtx:
									...
									for GRTParticle1 in This.RTTools.getParticles(GRTVertex1):
										...
										if (GRTParticle1.pdg == ParticleCodes.Proton):
											
											for Vertex1 in This.Truth_Vertices.Vertices:
										
												if (GRTVertex1.TruthVertexID == Vertex1.ID):
													...
													if (IncidentMuonNeutrino and ProtonFromDelta and PhotonFromDelta and TruthInteractionType == "RP"):
														EventInteraction = "DeltaPGamma"
													elif (IncidentMuonNeutrino and ProtonFromDelta and Pi0MesonFromDelta and TruthInteractionType == "RP"):
														EventInteraction = "DeltaPPi"
													elif (TruthInteractionType == "RP"):
														EventInteraction = "Other Resonance"
													else:
														EventInteraction = TruthInteractionType

													This.HistogramCollection1.Histograms["Proton_Location"].Populate(Vertex1.Subdetector)

													This.HistogramCollection1.Histograms["Proton_Location_Stacked"].Populate(EventInteraction, Vertex1.Subdetector, EventInteraction)
								...
								print "Warning: Could not complete Monte Carlo analysis for event" , i
								print "The following error occured:" , traceback.format_exc()
						...
						if (SuitableEvent and (This.NoSelection == False)):
							...
							try:
								...
								print "Warning: Unable to run event number" , i
								print "The following error occured:" , traceback.format_exc()
						...
						if ((OutputData or (i == EventNumber - 1)) and (SuitableEvents > 0)):
							...
							try:
								...
								for Key, SelectionChannel in This.SelectionDictionary.iteritems():
									...
									This.HistogramCollection1.Histograms[AbsoluteEfficiencyReference].Depopulate()
									This.HistogramCollection1.Histograms[RelativeEfficiencyReference].Depopulate()
									This.HistogramCollection1.Histograms[PurityReference].Depopulate()
									...
									This.HistogramCollection1.Histograms[NormalisedPurityReference].Depopulate()
									
									for CriteriaListCounter in xrange(CutNumber):
														
										if(SelectionChannel[CriteriaListCounter].Purity > 0):
											
											This.HistogramCollection1.Histograms[NormalisedPurityReference].PopulateWeighted("Delta -> p gamma interactions", CriteriaListCounter, SelectionChannel[CriteriaListCounter].Purity, "Delta-1 to a Proton and a Photon")
										
										if(SelectionChannel[CriteriaListCounter].PionPurity > 0):
											
											This.HistogramCollection1.Histograms[NormalisedPurityReference].PopulateWeighted("Delta -> p pi interactions", CriteriaListCounter, SelectionChannel[CriteriaListCounter].PionPurity, "Delta-1 to a Proton and a Pi-0")
									
										if(SelectionChannel[CriteriaListCounter].RPPurity > 0):
											
											This.HistogramCollection1.Histograms[NormalisedPurityReference].PopulateWeighted("Other Resonance Production", CriteriaListCounter, SelectionChannel[CriteriaListCounter].RPPurity, "Other Resonance")
								...
								############## Application of truth to criteria
									...
									for SelectionChannel1 in SelectionChannel:
										This.HistogramCollection1.Histograms[CutPurityReference].Populate("Delta -> p gamma Interactions", SelectionChannel1.TruthDelta, "Delta-1 to a Proton and a Photon")
										This.HistogramCollection1.Histograms[CutPurityReference].Populate("Delta -> p pi interactions", SelectionChannel1.ReconstructedDelta, "Delta-1 to a Proton and a Pi-0")							
										This.HistogramCollection1.Histograms[CutPurityReference].Populate("Other Resonance Production", SelectionChannel1.OtherRPInteraction, "Other Resonance")							
								...
								# Problem here when working on real data
								...
								if (This.MonteCarloData == False):
									This.GeneralStatistics.Statistics["NProtonsOnTargetData"].Quantity = This.ProtonsOnTargetCounterData
								else:
									This.GeneralStatistics.Statistics["NProtonsOnTargetMC"].Quantity = This.ProtonsOnTargetCounterMC
								...
								if (i == EventNumber - 1):
									...
									print This.GeneralStatistics.ToText()		
									...
									OutputDataFile.write(This.GeneralStatistics.ToText())
								...
								print "Warning: Histogram output at" , CurrentTime , "hours and event number" , i , "resulted in an error, histograms have not been outputted"
								print "The following error occured:" , traceback.format_exc()
				...
				print "Warning: could not load from data storage location" , i
				print "The following error occured:" , traceback.format_exc()
		
	def InitialiseStatistics(This):
	
		This.GeneralStatistics = Statistics.Collection("General Statistics")

		if (This.MonteCarloData == False):
			This.GeneralStatistics.NewStatistic("NProtonsOnTargetData", "Number of Protons on Target from Data")
		else:
			This.GeneralStatistics.NewStatistic("NProtonsOnTargetMC", "Number of Protons on Target from Monte Carlo")
		...
		This.TruthStatistics.NewStatistic("NEventsWithElectronPositronPairsNotFromDelta", "Number of events which contain electron-positron pairs (not from Delta)")
		...
		This.ReconstructedStatistics.NewStatistic("NPIDEventsFullMC", "Number of Events with at least one Reconstructed PID (out of all events)")
		This.ReconstructedStatistics.NewStatistic("NPIDEventsFullMCTPC", "Number of Events with at least one Reconstructed 18 node TPC PID (out of all events)")
		...
		This.ReconstructedStatistics.NewStatistic("NDeltaPGammaEventsWithRProtonTrack", "Number of Delta -> p gamma events with a single reconstructed proton track")
		This.ReconstructedStatistics.NewStatistic("NDeltaPGammaEventsWithNoRProtonTrack", "Number of Delta -> p gamma events with no reconstructed proton tracks")
		This.ReconstructedStatistics.NewStatistic("NDeltaPGammaEventsWithMultipleRProtonTrack", "Number of Delta -> p gamma events with multiple reconstructed proton tracks")
	...
	def InitialiseHistograms(This):
		...
		This.HistogramCollection1.NewHistogram1D("Reconstructed_Delta1Baryon_Energy", "DeltaBaryon/Reconstruction/", "Energy of the Unknown Particle which created the Reconstructed Proton and Photon", "Energy (GeV)", 100, 0, 5000, "Number")
		This.HistogramCollection1.NewHistogram1D("Reconstructed_Delta1Baryon_MomentumModulus", "DeltaBaryon/Reconstruction/", "Momentum Modulus of the Unknown Particle which created the Reconstructed Proton and Photon", "Momentum (GeV)", 100, 0, 3000, "Number")
		This.HistogramCollection1.NewHistogram1D("Reconstructed_Delta1Baryon_Mass", "DeltaBaryon/Reconstruction/", "Mass of the Unknown Particle which created the Reconstructed Proton and Photon", "Mass (GeV)", 100, 0, 3000, "Number")
		
		This.HistogramCollection1.NewHistogram1D("Reconstructed_Delta1Baryon_Energy_TimeSeparated", "DeltaBaryon/Reconstruction/", "Energy of the Unknown Particle which created the Reconstructed Proton and Photon (Time Separated)", "Energy (GeV)", 100, 0, 5000, "Number")
		This.HistogramCollection1.NewHistogram1D("Reconstructed_Delta1Baryon_MomentumModulus_TimeSeparated", "DeltaBaryon/Reconstruction/", "Momentum Modulus of the Unknown Particle which created the Reconstructed Proton and Photon (Time Separated)", "Momentum (GeV)", 100, 0, 3000, "Number")
		This.HistogramCollection1.NewHistogram1D("Reconstructed_Delta1Baryon_Mass_TimeSeparated", "DeltaBaryon/Reconstruction/", "Mass of the Unknown Particle which created the Reconstructed Proton and Photon (Time Separated)", "Mass (GeV)", 100, 0, 3000, "Number")
		...
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Proton_Recon_Momentum", "Proton/Reconstruction/Momentum/", "Reconstructed momentum of particles reconstructed as protons", "Momentum (GeV)", 100, 0, 3000, "Number")
			
		This.HistogramCollection1.NewHistogram1D("Recon_Truth_Proton_Momentum", "Objects/Reconstruction/Momentum", "Recon momentum minus truth momentum for PIDs reconstructed as protons", "Momentum (GeV)", 100, -500, 500, "Number")
		This.HistogramCollection1.NewHistogram1D("Truth_Proton_Momentum", "Objects/Truth/Momentum", "Truth momentum for PIDs reconstructed as protons", "Momentum (GeV)", 100, 0, 5000, "Number")
		
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Proton_Invariant_Mass_Stacked", "Proton/Reconstruction/Invariant Mass/", "Invariant Mass of Reconstructed Protons", "Mass (GeV)", 100, 930, 950, "Number")
		This.HistogramCollection1.NewHistogram1D("Recon_Proton_Invariant_Mass", "Proton/Reconstruction/Invariant Mass/", "Invariant Mass of Reconstructed Protons", "Mass (GeV)", 100, 930, 950, "Number")
		
		This.HistogramCollection1.NewHistogram1D("Recon_Proton_Multiplicity", "Proton/Reconstruction/Multiplicity/", "Proton Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Proton_Multiplicity_Stacked", "Proton/Reconstruction/Multiplicity/", "Proton Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		
		This.HistogramCollection1.NewHistogram1D("TrueProtonMultiplicity", "Proton/Truth/Multiplicity/", "Proton Track Multiplicity (Truth)", "Multiplicity", 15, 0, 15, "Number")
				
		This.HistogramCollection1.NewHistogram1D("Proton_Location", "Proton/Reconstruction/Location/", "Proton Location", "Detector Code", 10, 0, 10, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Proton_Location_Stacked", "Proton/Reconstruction/Location/", "Proton Location", "Detector Code", 10, 0, 10, "Number")
				
		############################################
		# Photon Data #
		############################################
				
		This.HistogramCollection1.NewHistogram1D("PhotonAngles", "Photons/Reconstruction/All/", "Distribution of the angle between the Photon Direction and the start of the Proton Track Position", "Angle", 100, 0, 180, "Number")
		This.HistogramCollection1.NewHistogram1D("ProtonPhotonTimeSeparations", "Photons/Reconstruction/", "Time Separation between the Reconstructed Proton Track and Photon EC Cluster", "Time Separation", 100, 0, 4000, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Photon_Angles_Stacked", "Photons/Reconstruction/All/", "Distribution of the angle between the Photon Direction and the start of the Proton Track Position", "Angle", 100, 0, 180, "Number")
		
		This.HistogramCollection1.NewHistogram1D("SelectedPhotonAngles", "Photons/Reconstruction/Selected/", "Distribution of the angle between the Photon Direction and the start of the Proton Track Position for the single proton and the single selected photon", "Angle", 100, 0, 180, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Selected_Photon_Angles_Stacked", "Photons/Reconstruction/Selected/", "Distribution of the angle between the Photon Direction and the start of the Proton Track Position for the single proton and the single selected photon", "Angle", 100, 0, 180, "Number")
		
		This.HistogramCollection1.NewStackedHistogram1D("Photon_Angles_Stacked_Track", "Photons/Reconstruction/Track/", "Distribution of the angle between the Photon Direction and the start of the Proton Track Position for the single proton and the single selected photon", "Angle", 100, 0, 180, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Photon_Angles_Stacked_Shower", "Photons/Reconstruction/Shower/", "Distribution of the angle between the Photon Direction and the start of the Proton Track Position for the single proton and the single selected photon", "Angle", 100, 0, 180, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Selected_Photon_Angles_Stacked_Track", "Photons/Reconstruction/Selected/Track/", "Distribution of the angle between the Photon Direction and the start of the Proton Track Position for the single proton and the single selected photon", "Angle", 100, 0, 180, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Selected_Photon_Angles_Stacked_Shower", "Photons/Reconstruction/Selected/Shower/", "Distribution of the angle between the Photon Direction and the start of the Proton Track Position for the single proton and the single selected photon", "Angle", 100, 0, 180, "Number")

		This.HistogramCollection1.NewHistogram1D("PID_Electron_Pull", "Objects/Reconstruction/Particle Pulls/", "Electron pull for every PID", "Electron Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("PID_Kaon_Pull", "Objects/Reconstruction/Particle Pulls/", "Kaon pull for every PID", "Kaon Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("PID_Muon_Pull", "Objects/Reconstruction/Particle Pulls/", "Muon pull for every PID", "Muon Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("PID_Pion_Pull", "Objects/Reconstruction/Particle Pulls/", "Pion pull for every PID", "Pion Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("PID_Proton_Pull", "Objects/Reconstruction/Particle Pulls/", "Proton pull for every PID", "Proton Pull", 100, -100, 100, "Number")
		
		This.HistogramCollection1.NewHistogram2D("PID_Proton_Against_Muon_Pull", "Objects/Reconstruction/Particle Pulls/", "Proton pull plotted against muon pull for every PID", "Proton Pull", 100, -100, 100, "Muon Pull", 100, -100, 100)
		
		This.HistogramCollection1.NewHistogram1D("Proton_PID_Electron_Pull", "Proton/Reconstruction/Particle Pulls/All Protons/", "Electron pull for all PIDs identified as proton-like", "Electron Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("Proton_PID_Kaon_Pull", "Proton/Reconstruction/Particle Pulls/All Protons/", "Kaon pull for all PIDs identified as proton-like", "Kaon Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("Proton_PID_Muon_Pull", "Proton/Reconstruction/Particle Pulls/All Protons/", "Muon pull for all PIDs identified as proton-like", "Muon Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("Proton_PID_Pion_Pull", "Proton/Reconstruction/Particle Pulls/All Protons/", "Pion pull for all PIDs identified as proton-like", "Pion Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("Proton_PID_Proton_Pull", "Proton/Reconstruction/Particle Pulls/All Protons/", "Proton pull for all PIDs identified as proton-like", "Proton Pull", 100, -100, 100, "Number")
		
		This.HistogramCollection1.NewHistogram2D("Proton_PID_Proton_Against_Muon_Pull", "Proton/Reconstruction/Particle Pulls/All Protons/", "Proton pull plotted against muon pull for all PIDs identified as proton-like", "Proton Pull", 100, -100, 100, "Muon Pull", 100, -100, 100)
		This.HistogramCollection1.Histograms["Proton_PID_Proton_Against_Muon_Pull"].MarkerStyle = 8
		
		This.HistogramCollection1.NewHistogram1D("Single_Proton_PID_Electron_Pull", "Proton/Reconstruction/Particle Pulls/Single Proton/", "Electron pull for single PIDs identified as proton-like", "Electron Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("Single_Proton_PID_Kaon_Pull", "Proton/Reconstruction/Particle Pulls/Single Proton/", "Kaon pull for single PIDs identified as proton-like", "Kaon Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("Single_Proton_PID_Muon_Pull", "Proton/Reconstruction/Particle Pulls/Single Proton/", "Muon pull for single PIDs identified as proton-like", "Muon Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("Single_Proton_PID_Pion_Pull", "Proton/Reconstruction/Particle Pulls/Single Proton/", "Pion pull for single PIDs identified as proton-like", "Pion Pull", 100, -100, 100, "Number")
		This.HistogramCollection1.NewHistogram1D("Single_Proton_PID_Proton_Pull", "Proton/Reconstruction/Particle Pulls/Single Proton/", "Proton pull for single PIDs identified as proton-like", "Proton Pull", 100, -100, 100, "Number")
		
		This.HistogramCollection1.NewHistogram2D("Single_Proton_PID_Proton_Against_Muon_Pull", "Proton/Reconstruction/Particle Pulls/Single Proton/", "Proton pull plotted against muon pull for single PIDs identified as proton-like", "Proton Pull", 100, -100, 100, "Muon Pull", 100, -100, 100)
		This.HistogramCollection1.Histograms["Proton_PID_Proton_Against_Muon_Pull"].MarkerStyle = 8
		
		This.HistogramCollection1.NewStackedHistogram1D("Proton_Photon_Synchronicity", "Photons/Reconstruction/Synchronicity/", "Distribution of the time separation between proton tracks and photon ECal clusters", "Time separation (ns)", 100, 0, This.Parameters["SynchronicityLimit"] * 5, "Number")
		
		This.HistogramCollection1.NewHistogram1D("Recon_ECal_Multiplicity", "Photons/Reconstruction/Multiplicity/", "ECal Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_ECal_Multiplicity_Stacked", "Photons/Reconstruction/Multiplicity/", "ECal Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
				
		############################################
		# Electron-Antielectron Data #
		############################################
		
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_SeparationX", "Photons/ElectronsAndAntielectrons/", "Electron positron separation in X axis", "Separation (mm)", This.Parameters["LocalityLimit"].X * 2, 0, This.Parameters["LocalityLimit"].X * 10, "Number")
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_SeparationY", "Photons/ElectronsAndAntielectrons/", "Electron positron separation in Y axis", "Separation (mm)", This.Parameters["LocalityLimit"].Y * 2, 0, This.Parameters["LocalityLimit"].Y * 10, "Number")
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_SeparationZ", "Photons/ElectronsAndAntielectrons/", "Electron positron separation in Z axis", "Separation (mm)", This.Parameters["LocalityLimit"].Z * 2, 0, This.Parameters["LocalityLimit"].Z * 10, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Separation_StackedX", "Photons/ElectronsAndAntielectrons/", "Separation in X axis between electrons and antielectrons", "Separation (mm)", This.Parameters["LocalityLimit"].X * 2, 0, This.Parameters["LocalityLimit"].X * 10, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Separation_StackedY", "Photons/ElectronsAndAntielectrons/", "Separation in Y axis between electrons and antielectrons", "Separation (mm)", This.Parameters["LocalityLimit"].Y * 2, 0, This.Parameters["LocalityLimit"].Y * 10, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Separation_StackedZ", "Photons/ElectronsAndAntielectrons/", "Separation in Z axis between electrons and antielectrons", "Separation (mm)", This.Parameters["LocalityLimit"].Z * 2, 0, This.Parameters["LocalityLimit"].Z * 10, "Number")
		
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Separation_StackedX_Process", "Photons/ElectronsAndAntielectrons/", "Separation in X axis between electrons and antielectrons", "Separation (mm)", This.Parameters["LocalityLimit"].X * 2, 0, This.Parameters["LocalityLimit"].X * 10, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Separation_StackedY_Process", "Photons/ElectronsAndAntielectrons/", "Separation in Y axis between electrons and antielectrons", "Separation (mm)", This.Parameters["LocalityLimit"].Y * 2, 0, This.Parameters["LocalityLimit"].Y * 10, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Separation_StackedZ_Process", "Photons/ElectronsAndAntielectrons/", "Separation in Z axis between electrons and antielectrons", "Separation (mm)", This.Parameters["LocalityLimit"].Z * 2, 0, This.Parameters["LocalityLimit"].Z * 10, "Number")
		
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Pair_Process", "Photons/ElectronsAndAntielectrons/", "Constituent processes of true electron positron pairs", "", 1, 0, 1, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_PairPhoton_Process", "Photons/ElectronsAndAntielectrons/", "Constituent processes of true electron positron pairs from photons", "", 1, 0, 1, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_CorrectlyReconstructed_Process", "Photons/ElectronsAndAntielectrons/", "Constituent processes of true electrons and positrons that are not pair produced", "", 1, 0, 1, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_ElectronCorrect_Process", "Photons/ElectronsAndAntielectrons/", "Constituent processes of electron positron PIDs where only the electron is correctly reconstructed", "", 1, 0, 1, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_PositronCorrect_Process", "Photons/ElectronsAndAntielectrons/", "Constituent processes of electron positron PIDs where only the positron is correctly reconstructed", "", 1, 0, 1, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Incorrect_Process", "Photons/ElectronsAndAntielectrons/", "Constituent processes of incorrectly identified identified electron and positron PIDs", "", 1, 0, 1, "Number")
		
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_InvariantMass", "Photons/ElectronsAndAntielectrons/Invariant Mass/", "Invariant Mass Distribution of all possible Electron-Positron Pairs", "Mass (MeV)", 100, 0, 100, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_InvariantMass_Stacked", "Photons/ElectronsAndAntielectrons/Invariant Mass/", "Invariant Mass Distribution of all possible Electron-Positron Pairs", "Mass (MeV)", 100, 0, 100, "Number")
		
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_WithinAngle_InvariantMass", "Photons/ElectronsAndAntielectrons/Invariant Mass/", "Invariant Mass Distribution of Electron-Positron Pairs", "Mass (MeV)", 100, 0, 100, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_WithinAngle_InvariantMass_Stacked", "Photons/ElectronsAndAntielectrons/Invariant Mass/", "Invariant Mass Distribution of Electron-Positron Pairs", "Mass (MeV)", 100, 0, 100, "Number")
		
		This.HistogramCollection1.NewHistogram1D("ElectronsAndAntielectrons_Angle", "Photons/ElectronsAndAntielectrons/Angle/", "Angular Distribution between Electron-Positron Pair direction and Pair-Proton Line", "Angle", 100, 0, 180, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Angle_Stacked", "Photons/ElectronsAndAntielectrons/Angle/", "Angular Distribution between Electron-Positron Pair direction and Pair-Proton Line", "Angle", 100, 0, 180, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Angle_StackedReconstruction", "Photons/ElectronsAndAntielectrons/Angle/", "Angular Distribution between Electron-Positron Pair direction and Pair-Proton Line", "Angle", 100, 0, 180, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("ElectronsAndAntielectrons_Angle_StackedReconstructionNoProton", "Photons/ElectronsAndAntielectrons/Angle/", "Angular Distribution between Electron-Positron Pair direction and Pair-Proton Line", "Angle", 100, 0, 180, "Number")
		...
		This.HistogramCollection1.NewHistogram1D("MuLeptons_Momentum", "MuLeptons/", "Momentum Moduli of the Reconstructed Mu Leptons", "Momentum (GeV)", 100, 0, 5000, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("MuLeptons_Momentum_WithTruthParticles", "MuLeptons/", "Momentum Moduli of the Reconstructed Mu Leptons", "Momentum (GeV)", 100, 0, 5000, "Number")	
		
		This.HistogramCollection1.NewHistogram1D("MuLeptonsAndProtons_Momentum", "MuLeptons/", "Momentum Moduli of the Reconstructed Mu Leptons and Protons", "Momentum (GeV)", 100, 0, 5000, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("MuLeptonsAndProtons_Momentum_WithTruthParticles", "MuLeptons/", "Momentum Moduli of the Reconstructed Mu Leptons and Protons", "Momentum (GeV)", 100, 0, 5000, "Number")	
		
		############################################
		# Multiplicities #
		############################################
		
		This.HistogramCollection1.NewHistogram1D("ProtonTrackMultiplicityECal", "Proton/Reconstruction/Multiplicity/", "Proton track multiplicity for events that have a proton and photon within angle and synchronicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewHistogram1D("ECalMultiplicityECal", "Photons/Reconstruction/Multiplicity/", "ECal multiplicity for events that have a proton and photon within angle and synchronicity", "Multiplicity", 15, 0, 15, "Number")
						
		This.HistogramCollection1.NewStackedHistogram1D("Single_Proton_PID_Multiplicity", "Proton/", "PID multiplicity for events with single proton tracks", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Single_Proton_ECal_Multiplicity", "Photons/", "Calorimeter multiplicity for events with single proton tracks", "Multiplicity", 15, 0, 15, "Number")	
		
		This.HistogramCollection1.NewHistogram1D("Recon_Track_Multiplicity", "PIDs/Reconstruction/Multiplicity/", "PID Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Track_Multiplicity_Stacked", "PIDs/Reconstruction/Multiplicity/", "PID Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		
		This.HistogramCollection1.NewHistogram1D("Recon_Track_Multiplicity_Before_TPC", "PIDs/Reconstruction/Multiplicity/", "PID Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Track_Multiplicity_Stacked_Before_TPC", "PIDs/Reconstruction/Multiplicity/", "PID Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		
		This.HistogramCollection1.NewHistogram1D("Recon_Electron_Multiplicity", "PIDs/Reconstruction/Multiplicity/Electron/", "Electron Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Electron_Multiplicity_Stacked", "PIDs/Reconstruction/Multiplicity/Electron/", "Electron Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		
		This.HistogramCollection1.NewHistogram1D("Recon_Positron_Multiplicity", "PIDs/Reconstruction/Multiplicity/Positron/", "Positron Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Positron_Multiplicity_Stacked", "PIDs/Reconstruction/Multiplicity/Positron/", "Positron Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		
		This.HistogramCollection1.NewHistogram1D("Recon_Muon_Multiplicity", "PIDs/Reconstruction/Multiplicity/Muon/", "Muon Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Muon_Multiplicity_Stacked", "PIDs/Reconstruction/Multiplicity/Muon/", "Muon Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		
		This.HistogramCollection1.NewStackedHistogram1D("Pair_ECal_Multiplicity", "PIDs/Reconstruction/Multiplicity/Pair/ECal/", "ECal Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Pair_Track_Multiplicity", "PIDs/Reconstruction/Multiplicity/Pair/Track/", "Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Pair_Track_Multiplicity", "PIDs/Reconstruction/Multiplicity/Pair/Track/", "Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Recon_Pair_Track_Multiplicity_C8", "PIDs/Reconstruction/Multiplicity/Pair/Track/", "Track Multiplicity", "Multiplicity", 15, 0, 15, "Number")
		
		# TPC Track Points #
		
		This.HistogramCollection1.NewHistogram1D("TPCTrackPoints_Multiplicities", "Subdetectors/TPCs/", "TPC Track Point Multiplicity", "Multiplicity", 100, 0, 100, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("TPCTrackPoints_Multiplicities_ByTrueParticle", "Subdetectors/TPCs/", "TPC Track Point Multiplicity by True Particle", "Multiplicity", 100, 0, 100, "Number")	
		This.HistogramCollection1.NewStackedHistogram1D("TPCTrackPoints_Multiplicities_ByReconstructedParticle", "Subdetectors/TPCs/", "TPC Track Point Multiplicity by Reconstructed Particle", "Multiplicity", 100, 0, 100, "Number")	
		
		############################################
		# Pulls #
		############################################
		
		This.HistogramCollection1.NewStackedHistogram1D("ElectronPull", "Objects/Reconstruction/Particle Pulls/", "Electron pull for all reconstructed objects", "Pull", 100, -100, 100, "Number")	
		This.HistogramCollection1.NewStackedHistogram1D("KaonPull", "Objects/Reconstruction/Particle Pulls/", "Kaon pull for all reconstructed objects", "Pull", 100, -100, 100, "Number")	
		This.HistogramCollection1.NewStackedHistogram1D("MuonPull", "Objects/Reconstruction/Particle Pulls/", "Muon pull for all reconstructed objects", "Pull", 100, -100, 100, "Number")	
		This.HistogramCollection1.NewStackedHistogram1D("PionPull", "Objects/Reconstruction/Particle Pulls/", "Pion pull for all reconstructed objects", "Pull", 100, -100, 100, "Number")	
		This.HistogramCollection1.NewStackedHistogram1D("ProtonPull", "Objects/Reconstruction/Particle Pulls/", "Proton pull for all reconstructed objects", "Pull", 100, -100, 100, "Number")	
		
		
		This.HistogramCollection1.NewStackedHistogram1D("Final_Invariant_MassECal", "Selections/DeltaPGamma/Invariant Mass/", "Invariant Mass of Delta Baryon after all cuts have been applied (ECal channel)", "Mass (GeV)", 100, 0, 3000, "Number")
		This.HistogramCollection1.NewStackedHistogram1D("Final_Invariant_MassPP", "Selections/DeltaPGamma/Invariant Mass/", "Invariant Mass of Delta Baryon after all cuts have been applied (Pair Production Channel)", "Mass (GeV)", 100, 0, 3000, "Number")
	...
	def runEvent(This):
		...
		if (This.MonteCarloData):
			...
			for Trajectory1 in This.Truth_Trajectories.Trajectories:
				...
				if(Trajectory1.PDG == ParticleCodes.Proton):
					
					ProtonParentList.append(Trajectory1.ParentID)#Creates a list of the parent trajectory IDs for all muons
			
			
			EventContainsElectronAntielectronPairsNotFromDelta = False
			PhotonTrajectoryIDs = []
			...
			for Vertex1 in This.Truth_Vertices.Vertices:
				...
				for Trajectory1 in This.Truth_Trajectories.Trajectories:#First loop over all trajectories in an event
					...
					for TrajectoryID in Vertex1.PrimaryTrajIDs:
				
						if (Trajectory1.ID == TrajectoryID):
				
							if (Trajectory1.PDG == ParticleCodes.Photon):
					
								PhotonTrajectoryIDs.append(Trajectory1.ID)
			...
			for ElectronParent in ElectronParentList:
				...
				for PositronParent in PositronParentList:
					...
					if (ElectronParent == PositronParent):#Finds Electron Positron pairs
						...
						for PhotonTrajectoryID in PhotonTrajectoryIDs:
							
							if (PhotonTrajectoryID == ElectronParent):#Finds if the electron positron pairs are from a 
														
								EventContainsElectronAntielectronPairsNotFromDelta = True		
			...
			if (EventContainsElectronAntielectronPairsNotFromDelta == True):	
				This.TruthStatistics.Statistics["NEventsWithElectronPositronPairsNotFromDelta"].Quantity += 1
			...
			This.HistogramCollection1.Histograms["TrueProtonMultiplicity"].Populate(len(ProtonParentList))
		...
		for PID in This.Reconstructed_Global.PIDs: # Loop over the PIDs if they exist
			...
			for TPCTrack1 in PID.TPC: # Loop over TPC PIDs	
				...
				This.HistogramCollection1.Histograms["TPCTrackPoints_Multiplicities"].Populate(TPCTrack1.NNodes)
				
				ReconstructedObject1.TPCTrackPoints.append(TPCTrack1.NNodes)
				
				if (TPCTrack1.NNodes > 18):
					SuitableTPCNodeNumber = True

			ReconstructedObject1.EDeposit = PID.EDeposit
			...
			if (SuitableTPCNodeNumber):
				...
				This.HistogramCollection1.Histograms["ElectronPull"].Populate(ReconstructedObject1.TrueParticleID, ReconstructedObject1.ParticlePull[ParticleCodes.Electron], str(ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID)))
				This.HistogramCollection1.Histograms["KaonPull"].Populate(ReconstructedObject1.TrueParticleID, ReconstructedObject1.ParticlePull[ParticleCodes.K1Meson], str(ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID)))
				This.HistogramCollection1.Histograms["MuonPull"].Populate(ReconstructedObject1.TrueParticleID, ReconstructedObject1.ParticlePull[ParticleCodes.MuLepton], str(ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID)))
				This.HistogramCollection1.Histograms["PionPull"].Populate(ReconstructedObject1.TrueParticleID, ReconstructedObject1.ParticlePull[ParticleCodes.Pi1Meson], str(ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID)))
				This.HistogramCollection1.Histograms["ProtonPull"].Populate(ReconstructedObject1.TrueParticleID, ReconstructedObject1.ParticlePull[ParticleCodes.Proton], str(ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID)))
		...
		for ReconstructedObject1 in ReconstructedObjects1:
			...
			for a1 in ReconstructedObject1.TPCTrackPoints:
				This.HistogramCollection1.Histograms["TPCTrackPoints_Multiplicities_ByTrueParticle"].Populate(ReconstructedObject1.TrueParticleID, a1, ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID))
				This.HistogramCollection1.Histograms["TPCTrackPoints_Multiplicities_ByReconstructedParticle"].Populate(ReconstructedObject1.ReconParticleID, a1, ParticleCodes.ParticleName(ReconstructedObject1.ReconParticleID))
			...
			if (ReconstructedObject1.ReconParticleID == ParticleCodes.Proton):#Looking for reconstructed proton track
				...
				This.HistogramCollection1.Histograms["Recon_Proton_Invariant_Mass"].Populate(Proton1.EnergyMomentum.InvariantModulus())
			
				if (This.MonteCarloData):
					This.HistogramCollection1.Histograms["Recon_Proton_Invariant_Mass_Stacked"].Populate(ReconstructedObject1.TrueParticleID, Proton1.EnergyMomentum.InvariantModulus(), ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID))
				...
				This.HistogramCollection1.Histograms["MuLeptonsAndProtons_Momentum"].Populate(Proton1.EnergyMomentum.SpatialModulus())
				This.HistogramCollection1.Histograms["MuLeptonsAndProtons_Momentum_WithTruthParticles"].Populate(ReconstructedObject1.TrueParticleID, Proton1.EnergyMomentum.SpatialModulus(), ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID))
			...
			if (ReconstructedObject1.ReconParticleID == ParticleCodes.MuLepton):
				...
				This.HistogramCollection1.Histograms["MuLeptons_Momentum"].Populate(Muon1.EnergyMomentum.SpatialModulus())
				This.HistogramCollection1.Histograms["MuLeptons_Momentum_WithTruthParticles"].Populate(ReconstructedObject1.TrueParticleID, Muon1.EnergyMomentum.SpatialModulus(), ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID))
				
				This.HistogramCollection1.Histograms["MuLeptonsAndProtons_Momentum"].Populate(Muon1.EnergyMomentum.SpatialModulus())
				This.HistogramCollection1.Histograms["MuLeptonsAndProtons_Momentum_WithTruthParticles"].Populate(ReconstructedObject1.TrueParticleID, Muon1.EnergyMomentum.SpatialModulus(), ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID))

			if (ReconstructedObject1.ReconParticleID == ParticleCodes.Electron and ReconstructedObject1.Charge == -1):
				NumberOfReconstructedElectronTracks += 1
				
				Electron1 = Particles.ReconstructedParticle()
		
				Electron1.Identification = ReconstructedObject1.Identification
		
				Electron1.EnergyMomentum.T = ReconstructedObject1.ReconstructedParticleEnergy()
				Electron1.EnergyMomentum.X = ReconstructedObject1.ReconFrontMomentum * ReconstructedObject1.ReconFrontDirectionX
				Electron1.EnergyMomentum.Y = ReconstructedObject1.ReconFrontMomentum * ReconstructedObject1.ReconFrontDirectionY
				Electron1.EnergyMomentum.Z = ReconstructedObject1.ReconFrontMomentum * ReconstructedObject1.ReconFrontDirectionZ
					
				Electron1.Position.T = ReconstructedObject1.FrontPosition.T
				Electron1.Position.X = ReconstructedObject1.FrontPosition.X
				Electron1.Position.Y = ReconstructedObject1.FrontPosition.Y
				Electron1.Position.Z = ReconstructedObject1.FrontPosition.Z
				
				Electron1.TrueTrajectory = ReconstructedObject1.TrueTrajectory
				Electron1.TrueGRTVertex = ReconstructedObject1.TrueGRTVertex					
				
				if (This.MonteCarloData):	
					Electron1.GetProcess(This.RTTools)
				
				ReconstructedElectrons1.append(Electron1)
				
			if (ReconstructedObject1.ReconParticleID == ParticleCodes.Electron and ReconstructedObject1.Charge == 1):
				NumberOfReconstructedAntiElectronTracks += 1
				
				Positron1 = Particles.ReconstructedParticle()
		
				Positron1.Identification = ReconstructedObject1.Identification
		
				Positron1.EnergyMomentum.T = ReconstructedObject1.ReconstructedParticleEnergy()
				Positron1.EnergyMomentum.X = ReconstructedObject1.ReconFrontMomentum * ReconstructedObject1.ReconFrontDirectionX
				Positron1.EnergyMomentum.Y = ReconstructedObject1.ReconFrontMomentum * ReconstructedObject1.ReconFrontDirectionY
				Positron1.EnergyMomentum.Z = ReconstructedObject1.ReconFrontMomentum * ReconstructedObject1.ReconFrontDirectionZ
					
				Positron1.Position.T = ReconstructedObject1.FrontPosition.T
				Positron1.Position.X = ReconstructedObject1.FrontPosition.X
				Positron1.Position.Y = ReconstructedObject1.FrontPosition.Y
				Positron1.Position.Z = ReconstructedObject1.FrontPosition.Z
				
				Positron1.TrueTrajectory = ReconstructedObject1.TrueTrajectory
				Positron1.TrueGRTVertex = ReconstructedObject1.TrueGRTVertex					
				
				if (This.MonteCarloData):	
					Positron1.GetProcess(This.RTTools)
				
				ReconstructedPositrons1.append(Positron1)
			...
			if (ReconstructedObject1.ReconParticleID == ParticleCodes.Proton):
				...
				This.HistogramCollection1.Histograms["Recon_Proton_True_Energy"].Populate(ReconstructedObject1.TrueParticleID, ReconstructedObject1.TrueEnergy, ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID))
				This.HistogramCollection1.Histograms["Recon_Proton_Recon_Momentum"].Populate(ReconstructedObject1.TrueParticleID, ReconstructedObject1.ReconFrontMomentum, ParticleCodes.ParticleName(ReconstructedObject1.TrueParticleID))
		...
		for ReconstructedProton1 in ReconstructedProtons1:
			...
			for ReconstructedPhoton1 in ReconstructedPhotons1:
				...
				This.HistogramCollection1.Histograms["Photon_Angles_Stacked"].Populate(Proton1.Process, Angle1, ReconstructedProton1.Process)
				...
				if ((math.fabs(Angle1) < math.fabs(SmallestAngle)) and (math.fabs(Angle1) < math.fabs(This.Parameters["PhotonAngularLocalityLimit"]))):
						
					NumberOfProtonsAndPhotonsWithinAngle += 1
					
					#######
					
				if ((This.FGD1Fiducial.Contains(ReconstructedProton1.Position.X, ReconstructedProton1.Position.Y, ReconstructedProton1.Position.Z) == True) or This.FGD2Fiducial.Contains(ReconstructedProton1.Position.X, ReconstructedProton1.Position.Y, ReconstructedProton1.Position.Z) == True):
					
					if (TimeSeparation < This.Parameters["SynchronicityLimit"]):
					
						if ((math.fabs(Angle1) < math.fabs(SmallestAngle)) and (math.fabs(Angle1) < math.fabs(This.Parameters["PhotonAngularLocalityLimit"]))):
							
							MultipleProtonsInBunch = False
							
							for ReconstructedProton2 in ReconstructedProtons1:	
								
								if (ReconstructedProton2 != ReconstructedProton1):
								
									TimeSeparation = math.fabs(float(ReconstructedProton1.Position.T) - float(ReconstructedProton2.Position.T))
									
									if (TimeSeparation < This.Parameters["SynchronicityLimit"]):
										
										MultipleProtonsInBunch = True

							if (MultipleProtonsInBunch == False):
								
								SelectedProtons1.append(ReconstructedProton1)
								
								SelectedPhotons1.append(ReconstructedPhoton1)			
		
		################ New Multiplicity Section ############
		
		ProtonBunchTrackMultiplicityList = []
		ProtonBunchMuonMultiplicityList = []
		ProtonBunchElectronMultiplicityList = []
		ProtonBunchPositronMultiplicityList = []
		ProtonBunchPhotonMultiplicityList = []
		LoopedOverProtons = False
				
		for Proton1 in SelectedProtons1:
			
			ProtonBunchTrackMultiplicity = 0
			ProtonBunchMuonMultiplicity = 0
			ProtonBunchElectronMultiplicity = 0
			ProtonBunchPositronMultiplicity = 0
			ProtonBunchPhotonMultiplicity = 0
			
			for ReconstructedObject1 in ReconstructedObjects1:
				
				TimeSeparation = math.fabs(float(ReconstructedProton1.Position.T) - float(ReconstructedObject1.FrontPosition.T))
				
				if (TimeSeparation < This.Parameters["SynchronicityLimit"]):
					
					ProtonBunchTrackMultiplicity += 1
					
			ProtonBunchTrackMultiplicityList.append(ProtonBunchTrackMultiplicity)
			
			#
			
			for ReconstructedMuLepton1 in ReconstructedMuLeptons1:
				
				TimeSeparation = math.fabs(float(ReconstructedProton1.Position.T) - float(ReconstructedMuLepton1.Position.T))
				
				if (TimeSeparation < This.Parameters["SynchronicityLimit"]):
					
					ProtonBunchMuonMultiplicity += 1
					
			ProtonBunchMuonMultiplicityList.append(ProtonBunchMuonMultiplicity)
			
			#
			
			for ReconstructedElectron1 in ReconstructedElectrons1:
				
				TimeSeparation = math.fabs(float(ReconstructedProton1.Position.T) - float(ReconstructedElectron1.Position.T))
				
				if (TimeSeparation < This.Parameters["SynchronicityLimit"]):
					
					ProtonBunchElectronMultiplicity += 1
					
			ProtonBunchElectronMultiplicityList.append(ProtonBunchElectronMultiplicity)
			
			#
			
			for ReconstructedPositron1 in ReconstructedPositrons1:
				
				TimeSeparation = math.fabs(float(ReconstructedProton1.Position.T) - float(ReconstructedPositron1.Position.T))
				
				if (TimeSeparation < This.Parameters["SynchronicityLimit"]):
					
					ProtonBunchPositronMultiplicity += 1
					
			ProtonBunchPositronMultiplicityList.append(ProtonBunchPositronMultiplicity)
			...
			for ReconstructedPhoton1 in ReconstructedPhotons1:
				...
				if (TimeSeparation < This.Parameters["SynchronicityLimit"]):
					...
					ProtonBunchPhotonMultiplicity += 1
					
			ProtonBunchPhotonMultiplicityList.append(ProtonBunchPhotonMultiplicity)
			
			LoopedOverProtons = True
		
		if (LoopedOverProtons):
			
			ProtonBunchTrackMultiplicityCondition = True
			ProtonBunchMuonMultiplicityCondition = True
			ProtonBunchElectronMultiplicityCondition = True
			ProtonBunchPositronMultiplicityCondition = True
			ProtonBunchPhotonMultiplicityCondition = True
				
			for ProtonBunchTrackMultiplicity in ProtonBunchTrackMultiplicityList:
				
				if (ProtonBunchTrackMultiplicity > This.Parameters["PIDTrackMultiplicityLimit"]):
					
					ProtonBunchTrackMultiplicityCondition = False
					
			for ProtonBunchMuonMultiplicity in ProtonBunchMuonMultiplicityList:
				
				if (ProtonBunchMuonMultiplicity > This.Parameters["MuonMultiplicityCut"]):
					
					ProtonBunchMuonMultiplicityCondition = False
					
			for ProtonBunchElectronMultiplicity in ProtonBunchElectronMultiplicityList:
				
				if (ProtonBunchElectronMultiplicity > This.Parameters["ElectronMultiplicityCut"]):
					
					ProtonBunchElectronMultiplicityCondition = False
					
			for ProtonBunchPositronMultiplicity in ProtonBunchPositronMultiplicityList:
				
				if (ProtonBunchPositronMultiplicity > This.Parameters["PositronMultiplicityCut"]):
					
					ProtonBunchPositronMultiplicityCondition = False
					
			for ProtonBunchPhotonMultiplicity in ProtonBunchPhotonMultiplicityList:
				
				if (ProtonBunchPhotonMultiplicity > This.Parameters["ECalMultiplicityLimit"]):
					
					ProtonBunchPhotonMultiplicityCondition = False
		...
		if (This.MonteCarloData):
			...
			if (TrueEventContainsDelta1ToProtonPhoton and TrueProtonPIDNumber == 1):
				This.ReconstructedStatistics.Statistics["NDeltaPGammaEventsWithRProtonTrack"].Quantity += 1
			elif (TrueEventContainsDelta1ToProtonPhoton and TrueProtonPIDNumber == 0):
				This.ReconstructedStatistics.Statistics["NDeltaPGammaEventsWithNoRProtonTrack"].Quantity += 1
			elif (TrueEventContainsDelta1ToProtonPhoton):
				This.ReconstructedStatistics.Statistics["NDeltaPGammaEventsWithMultipleRProtonTrack"].Quantity += 1
		
		This.HistogramCollection1.Histograms["Recon_Proton_Multiplicity"].Populate(NumberOfReconstructedProtonTracks)
		This.HistogramCollection1.Histograms["Recon_Track_Multiplicity"].Populate(TPCValidPIDNumber)			
		This.HistogramCollection1.Histograms["Recon_Track_Multiplicity_Before_TPC"].Populate(This.Reconstructed_Global.NPIDs)			
		This.HistogramCollection1.Histograms["Recon_ECal_Multiplicity"].Populate(NTrackerECalRecon)			
		This.HistogramCollection1.Histograms["Recon_Electron_Multiplicity"].Populate(NumberOfReconstructedElectronTracks)			
		This.HistogramCollection1.Histograms["Recon_Positron_Multiplicity"].Populate(NumberOfReconstructedAntiElectronTracks)			
		This.HistogramCollection1.Histograms["Recon_Muon_Multiplicity"].Populate(NumberOfReconstructedMuLeptonTracks)
		
		if (This.MonteCarloData):
			This.HistogramCollection1.Histograms["Recon_Proton_Multiplicity_Stacked"].Populate(TrueEventInteraction, NumberOfReconstructedProtonTracks, TrueEventInteraction)
			This.HistogramCollection1.Histograms["Recon_Track_Multiplicity_Stacked"].Populate(TrueEventInteraction, TPCValidPIDNumber, TrueEventInteraction)
			This.HistogramCollection1.Histograms["Recon_Track_Multiplicity_Stacked_Before_TPC"].Populate(TrueEventInteraction, This.Reconstructed_Global.NPIDs, TrueEventInteraction)
			This.HistogramCollection1.Histograms["Recon_ECal_Multiplicity_Stacked"].Populate(TrueEventInteraction, NTrackerECalRecon, TrueEventInteraction)	
			This.HistogramCollection1.Histograms["Recon_Electron_Multiplicity_Stacked"].Populate(TrueEventInteraction, NumberOfReconstructedElectronTracks, TrueEventInteraction)	
			This.HistogramCollection1.Histograms["Recon_Positron_Multiplicity_Stacked"].Populate(TrueEventInteraction, NumberOfReconstructedAntiElectronTracks, TrueEventInteraction)	
			This.HistogramCollection1.Histograms["Recon_Muon_Multiplicity_Stacked"].Populate(TrueEventInteraction, NumberOfReconstructedMuLeptonTracks, TrueEventInteraction)
			
			if(EventContainsElectronAntielectronPairs):
				This.HistogramCollection1.Histograms["Pair_ECal_Multiplicity"].Populate(TrueEventInteraction, NTrackerECalRecon, TrueEventInteraction)
				This.HistogramCollection1.Histograms["Pair_Track_Multiplicity"].Populate(TrueEventInteraction, TPCValidPIDNumber, TrueEventInteraction)
		...
		if ((NTrackerECalRecon > 0) and DeltaPGammaCriteria[4]):
			DeltaPGammaCriteria[5] = True
		
		if ((NumberOfProtonsAndPhotonsWithinSynchronicity > 0) and DeltaPGammaCriteria[5]):
			DeltaPGammaCriteria[6] = True
			
		if ((NumberOfProtonsAndPhotonsWithinAngle > 0) and DeltaPGammaCriteria[6]):
			DeltaPGammaCriteria[7] = True
			
		if (ProtonBunchTrackMultiplicityCondition and DeltaPGammaCriteria[7]):
			DeltaPGammaCriteria[8] = True
			
		if (ProtonBunchPhotonMultiplicityCondition and DeltaPGammaCriteria[8]):
			DeltaPGammaCriteria[9] = True
			
		if (ProtonBunchMuonMultiplicityCondition and DeltaPGammaCriteria[9]):
			DeltaPGammaCriteria[10] = True
		
		if (ProtonBunchElectronMultiplicityCondition and DeltaPGammaCriteria[10]):
			DeltaPGammaCriteria[11] = True	
		
		if (ProtonBunchPositronMultiplicityCondition and DeltaPGammaCriteria[11]):
		...
		#	ProtonPhotonToElectrons[5] = True
			...
			This.TrueEventInteraction = TrueEventInteraction
			(EventContainsElectronPositronPair, ElectronPositronPairPointsWithinAngle, ElectronPositronPairPointsWithinTime, SelectedProtonsPP, SelectedPhotonsPP) = This.EventContainsElectronPositron(ReconstructedObjects1, ReconstructedProtons1, None)
			...
			for Proton1 in SelectedProtonsPP:
...
###############################
		...
		if (ElectronPositronPairPointsWithinTime >= 1 and ProtonPhotonToElectrons[5]):
		...
		if (ProtonBunchTrackMultiplicityCondition and ProtonPhotonToElectrons[7]):
			ProtonPhotonToElectrons[8] = True
			
		if (ProtonBunchPhotonMultiplicityCondition and ProtonPhotonToElectrons[8]):
			ProtonPhotonToElectrons[9] = True
			
		if (ProtonBunchMuonMultiplicityCondition and ProtonPhotonToElectrons[9]):
		...
		if (DeltaPGammaCriteria[len(DeltaPGammaCriteria) - 1] == True):# !! Improve
			
			for Photon1 in SelectedPhotons1:
				
				for Proton1 in SelectedProtons1:
					
					DeltaBaryon = Geometry.FourVector()
					
					DeltaBaryon = Photon1.EnergyMomentum + Proton1.EnergyMomentum
			
					This.HistogramCollection1.Histograms["Final_Invariant_MassECal"].Populate(TrueEventInteraction, DeltaBaryon.InvariantModulus(), str(TrueEventInteraction))
					
		if (ProtonPhotonToElectrons[len(ProtonPhotonToElectrons) - 1] == True):
			
			for Photon1 in SelectedPhotonsPP:
				...
				for Proton1 in SelectedProtonsPP:
					...
					This.HistogramCollection1.Histograms["Final_Invariant_MassPP"].Populate(TrueEventInteraction, DeltaBaryon.InvariantModulus(), str(TrueEventInteraction))
		...
		if (EventContainsElectronPositronPair):
			
			This.HistogramCollection1.Histograms["Recon_Pair_Track_Multiplicity"].Populate(TrueEventInteraction, TPCValidPIDNumber, TrueEventInteraction)
			
		if (ProtonPhotonToElectrons[7]):
			This.HistogramCollection1.Histograms["Recon_Pair_Track_Multiplicity_C8"].Populate(TrueEventInteraction, TPCValidPIDNumber, TrueEventInteraction)
	...
	def EventContainsElectronPositron(This, PIDParticles1, Protons, TruthInteractionType):
		...
		for Electron1 in Electrons:
			for Positron1 in Positrons:
				...
				if (This.MonteCarloData):
					...
					This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Separation_StackedX_Process"].Populate(This.TrueEventInteraction, math.fabs(Separation.X), This.TrueEventInteraction)
					This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Separation_StackedY_Process"].Populate(This.TrueEventInteraction, math.fabs(Separation.Y), This.TrueEventInteraction)
					This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Separation_StackedZ_Process"].Populate(This.TrueEventInteraction, math.fabs(Separation.Z), This.TrueEventInteraction)
					...
					if ((Electron1.CorrectlyReconstructed() == True) and (Positron1.CorrectlyReconstructed() == True) and PairProduced):
						...
						This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Pair_Process"].Populate(This.TrueEventInteraction, 0.5, This.TrueEventInteraction)
						...
						This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_CorrectlyReconstructed_Process"].Populate(This.TrueEventInteraction, 0.5, This.TrueEventInteraction)
						...
						This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_ElectronCorrect_Process"].Populate(This.TrueEventInteraction, 0.5, This.TrueEventInteraction)
						...
						This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_PositronCorrect_Process"].Populate(This.TrueEventInteraction, 0.5, This.TrueEventInteraction)
					else:
						...
						This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Incorrect_Process"].Populate(This.TrueEventInteraction, 0.5, This.TrueEventInteraction)
				...
				if (Geometry.WithinLocality(Electron1.FrontPosition, Positron1.FrontPosition, This.Parameters["LocalityLimit"])):
					...
					if (This.MonteCarloData):
						This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_InvariantMass_Stacked"].Populate(This.TrueEventInteraction, PossiblePhoton1.EnergyMomentum.InvariantModulus(), This.TrueEventInteraction)
					...
					for Proton1 in Protons:
						...
						This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Angle"].Populate(Angle1)
						
						if (This.MonteCarloData):
							This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Angle_Stacked"].Populate(This.TrueEventInteraction, Angle1, This.TrueEventInteraction)
							ProtonCorrectlyReconstructed = False
							for Trajectory1 in This.Truth_Trajectories.Trajectories:
								
								if (Trajectory1.ID == Proton1.TrueTrajectory.ID):
									
									if (Trajectory1.PDG == ParticleCodes.Proton):
										
										ProtonCorrectlyReconstructed = True

							if (ProtonCorrectlyReconstructed == True):
								if (PairProduced):
									This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Angle_StackedReconstruction"].Populate("0", Angle1, "Electron Positron pair with Proton")
								else:
									This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Angle_StackedReconstruction"].Populate("1", Angle1, "Not Electron Positron with Proton")
									
							if (ProtonCorrectlyReconstructed == False):
								if (PairProduced):
									This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Angle_StackedReconstruction"].Populate("2", Angle1, "Electron Positron pair without Proton")
								else:
									This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Angle_StackedReconstruction"].Populate("3", Angle1, "Not Electron Positron without Proton")

							if (PairProduced):
								This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Angle_StackedReconstructionNoProton"].Populate("0", Angle1, "Electron Positron pair")
							else:
								This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_Angle_StackedReconstructionNoProton"].Populate("1", Angle1, "Not Electron Positron")
						...
						if ((This.FGD1Fiducial.Contains(Proton1.Position.X, Proton1.Position.Y, Proton1.Position.Z) == True) or This.FGD2Fiducial.Contains(Proton1.Position.X, Proton1.Position.Y, Proton1.Position.Z) == True):
							...
							if (TimeSeparation < This.Parameters["SynchronicityLimit"]):
								...
								if (Angle1 < This.Parameters["PairProductionAngularLocalityLimit"]):
									...
									for Proton2 in Protons:	
										
										if (Proton2 != Proton1):
										
											TimeSeparation = math.fabs(float(Proton1.Position.T) - float(Proton2.Position.T))
									...
									if (MultipleProtonsInBunch == False):
										...
										SelectedProtons.append(Proton1)
										SelectedPhotons.append(PossiblePhoton1)
													
									This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_WithinAngle_InvariantMass"].Populate(PossiblePhoton1.EnergyMomentum.InvariantModulus())
									
									if (This.MonteCarloData):
										This.HistogramCollection1.Histograms["ElectronsAndAntielectrons_WithinAngle_InvariantMass_Stacked"].Populate(This.TrueEventInteraction, PossiblePhoton1.EnergyMomentum.InvariantModulus(), This.TrueEventInteraction)
		...
		return (EventContainsElectronPositronPair, EventContainsElectronPositronPairWithinAngle, EventContainsElectronPositronPairWithinTime, SelectedProtons, SelectedPhotons)
		
		
	def IsValidSpill(This):
		
		ValidSpill = False

		if (This.MonteCarloData == True):
			ValidSpill = True
			
		else:

			if (This.BasicDataQuality.ND280OffFlag == 1 and This.BeamSummaryData.BeamSummaryDataStatus == 1):
				ValidSpill = True

				for BeamSummaryData1 in This.BeamSummaryData.BeamSummaryData:

					if (BeamSummaryData1.GoodSpillFlag != 1):

						ValidSpill = False

		return ValidSpill
		
	def ProtonsOnTargetData(This):
		
		ProtonsOnTargetData = 0

		for BeamSummaryData1 in This.BeamSummaryData.BeamSummaryData:

			ProtonsOnTargetData = BeamSummaryData1.CT5ProtonsPerSpill
				
		return ProtonsOnTargetData
				
	def ProtonsOnTargetMC(This):
		
		ProtonsOnTargetMC = 0
		
		if (This.Truth_GRTVertices.NVtx > 0):
				
				ProtonsOnTargetMC = This.Truth_GRTVertices.Vtx[0].OrigTreePOT
				
		return ProtonsOnTargetMC			
...
def main():
	...
	DefaultInputFileLocatorRealData1 = "/storage/epp2/t2k/data/nd280/production005/B/rdp/ND280/00006000_00006999/anal/"
	DefaultInputFileLocatorRealData2 = "/storage/epp2/t2k/data/nd280/production005/B/rdp/ND280/00007000_00007999/anal/"
	...
	NoSelection = False
	MonteCarloData = True# Because default is pure MC
	
	InputFileLocatorList = glob.glob(DefaultInputFileLocatorPurified + "*.root")
	...
	SimulationInformation["MonteCarloData"] = ("Monte Carlo Data", "MC")
				...
				if (len(ArgumentComponents) == 2):
					...
					if (ArgumentComponents[0] == "P"):
						if (ArgumentComponents[1] == str(0)):
							...
							InputFileLocatorList = glob.glob(DefaultInputFileLocatorUnpurified + "*.root")
							...
							SimulationInformation["InputFilePurity"] = ("Reading events from:" + str(DefaultInputFileLocatorRealData1) + ", " + str(DefaultInputFileLocatorRealData2), "P2")
							
							MonteCarloData = False
							SimulationInformation["MonteCarloData"] = ("Real Data", "R")
							
							InputFileLocatorList1 = glob.glob(DefaultInputFileLocatorRealData1 + "*.root")
							InputFileLocatorList2 = glob.glob(DefaultInputFileLocatorRealData2 + "*.root")
							
							InputFileLocatorList = InputFileLocatorList1 + InputFileLocatorList2
	...
	Analysis1.MonteCarloData = MonteCarloData
