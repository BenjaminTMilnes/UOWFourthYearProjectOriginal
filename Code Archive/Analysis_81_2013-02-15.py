...
class Analysis:
	...
	def __init__(This, InputFileLocatorList, OutputROOTFileLocator, SimulationInformationFileLocator, SimulationInformation):
		...
		This.Parameters["ECalMultiplicityLimitECalChannel"] = 3
	...
	def runEvent(This):
		...
		for PID in This.Reconstructed_Global.PIDs: # Loop over the PIDs if they exist
			...
			NumberOfTPCPoints = 0
			SuitableTPCNodeNumber = False #!!!! Warning, I have changed this temporarily as the TPC condition always seemed to make purities worse. I will put it back in if there is little change to purities.
		...
		if ((NTrackerECalRecon > 0) and DeltaPGammaCriteria[5]):
			DeltaPGammaCriteria[6] = True
		
		if ((NumberOfProtonsAndPhotonsWithinSynchronicity > 0) and DeltaPGammaCriteria[6]):
			DeltaPGammaCriteria[7] = True
			
		if ((NumberOfProtonsAndPhotonsWithinAngle > 0) and DeltaPGammaCriteria[7]):
			DeltaPGammaCriteria[8] = True
			
		if (NumberOfReconstructedProtonTracks <= This.Parameters["ProtonTrackMultiplicityLimitECalChannel"] and DeltaPGammaCriteria[8]):
			DeltaPGammaCriteria[9] = True
			
		if (NTrackerECalRecon <= This.Parameters["ECalMultiplicityLimitECalChannel"] and DeltaPGammaCriteria[9]):
		...
		ProtonPhotonToElectrons = [True, False, False, False, False, False, False, False, False, False, False, False]
		...
		if (ElectronPositronPairPointsWithinAngle >= 1 and ProtonPhotonToElectrons[7]):
			ProtonPhotonToElectrons[8] = True
		
		if (ElectronPositronPairPointsWithinTime >= 1 and ProtonPhotonToElectrons[8]):
			ProtonPhotonToElectrons[9] = True
			
		if (NumberOfReconstructedProtonTracks <= This.Parameters["ProtonTrackMultiplicityLimitPPChannel"] and ProtonPhotonToElectrons[9]):
			ProtonPhotonToElectrons[10] = True
			
		if (NTrackerECalRecon <= This.Parameters["ECalMultiplicityLimitPPChannel"] and ProtonPhotonToElectrons[10]):
