...
class Analysis:
	...
	def runEvent(This):
		...
		if (NumberOfReconstructedMuLeptonTracks > 0 and MuonTrackIsHighestEnergy and DeltaPGammaCriteria[7]):
			DeltaPGammaCriteria[8] = True
			
		if (MuonProtonMeet and DeltaPGammaCriteria[8]):
			DeltaPGammaCriteria[9] = True
		
		if ((NTrackerECalRecon > 0) and DeltaPGammaCriteria[9]):
			DeltaPGammaCriteria[10] = True

		if (SmallestAngle <= NumericalCuts.MinimumAngle and DeltaPGammaCriteria[10]):
			DeltaPGammaCriteria[11] = True
			
		if (SatisfiesInvariantMass and DeltaPGammaCriteria[11]):
		...
		if (This.SelectBackgroundEvents == True):
			if (DeltaPGammaCriteria[len(DeltaPGammaCriteria) - 1] == True):
				...
				if (SelectedProton.Process == "DeltaPPi"):
				...
				if ((SelectedProton.Process == "RP") and (not TrueEventContainsDelta1ToProtonPhoton) and (not TrueEventContainsDelta1ToProtonPi0)):
