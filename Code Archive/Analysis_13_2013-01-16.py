...
class Analysis:
	...
	def runEvent(self):
		...
		ProtonEnergyMomentumList = []
		ProtonFrontPositionList = []
		...
		for PIDObject1 in PIDObjectList:
			...
			if (PIDObject1.ReconParticleID == ParticleCodes.Proton):#Looking for reconstructed proton track
				...
				if (PIDObject1.Detectors[0] == "4" or PIDObject1.Detectors[0] == "5"):
					...
					FourVector1 = FourVector.FourVector()
					FourVector1.T = PIDObject1.ReconstructedParticleEnergy()
					FourVector1.X = PIDObject1.ReconFrontMomentum * PIDObject1.ReconFrontDirectionX
					FourVector1.Y = PIDObject1.ReconFrontMomentum * PIDObject1.ReconFrontDirectionY
					FourVector1.Z = PIDObject1.ReconFrontMomentum * PIDObject1.ReconFrontDirectionZ
					
					ProtonEnergyMomentumList.append(FourVector1)
					...
					ThreeVector1 = ThreeVector.ThreeVector()
					ThreeVector1.X = PIDObject1.ReconFrontPositionX
					ThreeVector1.Y = PIDObject1.ReconFrontPositionY
					ThreeVector1.Z = PIDObject1.ReconFrontPositionZ
					
					ProtonFrontPositionList.append(ThreeVector1)
		...
		for TECObject in self.Reconstructed_TEC.ReconObject:#Loop over these reconstructed objects
			...
			if (ECUnitDirection.Modulus() < 1.1):#For some reason there sometimes are occurences where the magnitude is far above 1. This if filters them out. We should probably look into this further
				...
				FourVector1.X = ECUnitDirection.X * ECEnergy
				FourVector1.Y = ECUnitDirection.Y * ECEnergy
				FourVector1.Z = ECUnitDirection.Z * ECEnergy
			
				PhotonEnergyMomentumList.append(FourVector1)
		...
		for Photon1 in PhotonList:
			for ProtonPosition1 in ProtonFrontPositionList:
		...
		if (ReconstructedProtonTrack and NTrackerECalRecon > 0 and Angle1 < 100):
		...
		if (len(PhotonEnergyMomentumList) > 0 and len(ProtonEnergyMomentumList) > 0):
			# If both a photon cluster and proton track were found ...
			
			for PhotonEnergy1 in PhotonEnergyMomentumList:
				for ProtonEnergy1 in ProtonEnergyMomentumList:
					...
					DeltaHadron.T = PhotonEnergy1.T + ProtonEnergy1.T # Delta Hadron Energy
					DeltaHadron.X = PhotonEnergy1.X + ProtonEnergy1.X
					DeltaHadron.Y = PhotonEnergy1.Y + ProtonEnergy1.Y
					DeltaHadron.Z = PhotonEnergy1.Z + ProtonEnergy1.Z
