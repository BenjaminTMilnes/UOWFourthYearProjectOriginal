...
class Analysis:
	...
	def runEvent(self):
		...
		for PIDObject1 in PIDObjectList:
			...
			if (PIDObject1.ReconParticleID == ParticleCodes.Proton and PIDObject1.TrueParticleID == ParticleCodes.MuLepton):
		...
		EventProcess = DictionaryKeys.EventProcess(DeltaPGammaEvent, Delta1BaryonToProtonPi0MesonEvent, InteractionType)
		
		if (Criteria[len(Criteria) - 1] == True):
			
			for Process1, Histogram1 in DictionaryKeys.TrueProcessDictionary.iteritems():
			
				if (EventProcess == Process1):
					
					try:
						self.FinalInvariantMass.HistogramDictionary[Process1].Histogram.Fill(DeltaBaryon.InvariantModulus())
					except:
						Title = "Invariant Mass contribution from true " + Histogram1.Reference
						self.FinalInvariantMass.NewHistogram1D(Process1, Title, Histogram1.XAxisTitle, Histogram1.YAxisTitle, Histogram1.NumberOfBins, Histogram1.MinimumX, Histogram1.MaximumX, Histogram1.Reference, "Cuts/Invariant Mass/Constituent Histograms/")
						self.FinalInvariantMass.HistogramDictionary[Process1].Histogram.Fill(DeltaBaryon.InvariantModulus())
