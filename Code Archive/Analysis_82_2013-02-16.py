...
class Analysis:
		...
		for i in range(EventNumber):
			...
			if (This.InputTimeTest == False):
				...
				if ((OutputData or (i == EventNumber - 1)) and (SuitableEvents > 0)):
					...
					This.EfficiencyStatistics_Delta1ToProtonPhotonPP.Statistics["NEvents7EventsRemaining"].Title = "Selection 7 - Event contains an electron and positron pair that begin within " + str(This.Parameters["LocalityLimit"]) + " cm of each other - Events Remaining"
					...
					This.EfficiencyStatistics_Delta1ToProtonPhotonPP.Statistics["NEvents7Delta"].Title = "Selection 7 - Event contains an electron and positron pair that begin within " + str(This.Parameters["LocalityLimit"]) + " cm of each other - Delta -> p gamma Events Remaining"
	...
	def runEvent(This):
		...
		for PID in This.Reconstructed_Global.PIDs: # Loop over the PIDs if they exist
			...
			#NumberOfTPCPoints = 0
			#MostTPCNodes = 0
			SuitableTPCNodeNumber = True #!!!! Warning, I have changed this temporarily as the TPC condition always seemed to make purities worse. I will put it back in if there is little change to purities.
										
			#for TPCTrack1 in PID.TPC: # Loop over TPC PIDs						
			#	if (TPCTrack1.NNodes > 0):
			...
			#		SuitableTPCNodeNumber = True
				...
				#if (TPCTrack1.NNodes > MostTPCNodes):
			...
			ReconstructedObject1.GoodNumberOfTPCPoints = True ##!!!!!
			...
			if (SuitableTPCNodeNumber):
				...
				for i, TPCTrack1 in enumerate(PID.TPC):
					if (TPCTrack1.NNodes > TPCNodeNumber):
						
						TPCNodeNumber = TPCTrack1.NNodes#!! Correction here, this loop was not working
						BestTPCTrack = i
						
				for i, FGDTrack1 in enumerate(PID.FGD):
					if (FGDTrack1.NNodes > FGDNodeNumber):
						
						FGDNodeNumber = FGDTrack1.NNodes#!! Correction here, this loop was not working
						BestFGDTrack = i

				if ((TPCNodeNumber >= FGDNodeNumber) and (TPCNodeNumber > 0)):#Assuming that TPC tracks are better
				...
				elif (TPCNodeNumber < FGDNodeNumber):

					for j , FGDTrack1 in enumerate(PID.FGD):
						if (j == BestFGDTrack):
							ReconstructedObject1.ParticlePull[ParticleCodes.Electron] = -999
							ReconstructedObject1.ParticlePull[ParticleCodes.K1Meson] = -999
							ReconstructedObject1.ParticlePull[ParticleCodes.MuLepton] = FGDTrack1.PullMuon
							ReconstructedObject1.ParticlePull[ParticleCodes.Pi1Meson] = FGDTrack1.PullPion
							ReconstructedObject1.ParticlePull[ParticleCodes.Proton] = FGDTrack1.PullProton
							
				else:
					
					ReconstructedObject1.ParticlePull[ParticleCodes.Electron] = - 999
					ReconstructedObject1.ParticlePull[ParticleCodes.K1Meson] = - 999
					ReconstructedObject1.ParticlePull[ParticleCodes.MuLepton] = - 999
					ReconstructedObject1.ParticlePull[ParticleCodes.Pi1Meson] = - 999
					ReconstructedObject1.ParticlePull[ParticleCodes.Proton] = - 999
		...
		#if (NumberOfReconstructedMuLeptonTracks == 0 and DeltaPGammaCriteria[4]):
		...
		if (NumberOfReconstructedProtonTracks <= This.Parameters["ProtonTrackMultiplicityLimitECalChannel"] and DeltaPGammaCriteria[7]):
			DeltaPGammaCriteria[8] = True
			
		if (NTrackerECalRecon <= This.Parameters["ECalMultiplicityLimitECalChannel"] and DeltaPGammaCriteria[8]):
		...
		if (NumberOfReconstructedProtonTracks <= This.Parameters["ProtonTrackMultiplicityLimitPPChannel"] and ProtonPhotonToElectrons[7]):
			ProtonPhotonToElectrons[8] = True
			
		if (NTrackerECalRecon <= This.Parameters["ECalMultiplicityLimitPPChannel"] and ProtonPhotonToElectrons[8]):
