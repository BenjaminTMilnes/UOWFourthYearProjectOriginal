...
class Analysis:
	...
	def Analyse(self, n = 999999999):
		...
		self.ROOTFile1.NewHistogram1D("Delta_Energy_Recon", "Energy of unknown parent particle of reconstructed proton and photon", "", "",100,0,5000)
		self.ROOTFile1.NewHistogram1D("Delta_Energy_True", "Energy of Delta Baryons", "", "",100,0,5000)
		self.ROOTFile1.NewHistogram1D("Delta_Momentum_Recon", "Energy of unknown parent particle of reconstructed proton and photon", "", "",100,0,3000)
		self.ROOTFile1.NewHistogram1D("Delta_Momentum_True", "Energy of Delta Baryons", "", "",100,0,3000)
		self.ROOTFile1.NewHistogram1D("Delta_Mass_Recon", "Mass of the Delta Baryon", "", "",100,0,3000)
		...
		self.ROOTFile1.NewHistogram1DStack("Cut_Purity","Constituent processes for each cut")
	...
	def runEvent(self):
		...
		for TrackerECalReconObject in TrackerECal.ReconObject:#Loop over these reconstructed objects
		...
		if (len(PhotonEnergyList)>0 and len(ProtonEnergyList)>0):#Including only events where both photon and proton reconstruction happened
			for PhotonEnergyCounter in xrange(len(PhotonEnergyList)):#Loop over every item in both proton and photon lists and match them up
				for ProtonEnergyCounter in xrange(len(ProtonEnergyList)):
					...
					self.ROOTFile1.Histograms["Delta_Energy_Recon"].Fill(UnknownObjectEnergy)
					...
					if (PhotonEnergyCounter<len(PhotonMomentumListX) and ProtonEnergyCounter<len(ProtonMomentumListX)):#To account for earlier trick of not including unit vectors > 1.1
						...
						if (UnknownObjectEnergy>DeltaMomentumMagnitude):#As this is statistical approach, sometimes can get sqrt(negative number)
							...
							self.ROOTFile1.Histograms["Delta_Mass_Recon"].Fill(DeltaMass)
										
						self.ROOTFile1.Histograms["Delta_Momentum_Recon"].Fill(DeltaMomentumMagnitude)
			...
			for GRTVertex1 in self.Truth_GRTVertices.Vtx: # Can loop over truth for comparison
				for GRTParticle1 in self.RTTools.getParticles(GRTVertex1):
					...
					if (GRTParticle1.pdg == ParticleCodes.Delta1Hadron):
						...
						self.ROOTFile1.Histograms["Delta_Energy_True"].Fill(DeltaEnergy)
						self.ROOTFile1.Histograms["Delta_Momentum_True"].Fill(DeltaMomentumMagnitudeTrue)
