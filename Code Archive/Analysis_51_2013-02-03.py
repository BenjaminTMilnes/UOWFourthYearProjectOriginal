...
class Analysis:
	...
	def Analyse(This, DesiredEventNumber = 999999999):
		...
		This.EfficiencyStatistics_Delta1ToProtonPhoton.NewStatistic("NEvents8")
		This.EfficiencyStatistics_Delta1ToProtonPhoton.NewStatistic("NEvents9", "Selection 8 - At least one EC cluster was recorded - Events Remaining")
		This.EfficiencyStatistics_Delta1ToProtonPhoton.NewStatistic("NEvents10")
		This.EfficiencyStatistics_Delta1ToProtonPhoton.NewStatistic("NEvents11")
		...
		if ((This.InputTimeTest == False) and (SuitableEvents > 0)):
			...
			for Key, SelectionChannel in This.SelectionDictionary.iteritems():
				...
				for CriteriaListCounter in xrange(CutNumber):
					...
					if	(Key == "DeltaPGamma"):
						...
						This.ROOTFile1.HistogramDictionary[PurityReference].Histogram.SetBinError(CriteriaListCounter + 1, PurityError)
				...
				This.ROOTFile1.HistogramDictionary[PurityReference].Histogram.SetOption("E1")
			...
			This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents8"].Title = "Selection 8 - If a Muon is present, its track begins within " + str(NumericalCuts.MuonProtonSeparation) + " cm of the start of the Proton track - Events Remaining"
			...
			This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents10"].Title = "Selection 9 - The smallest angle was less than " + str(NumericalCuts.MinimumAngle) + " degees - Events Remaining"
			...
			This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents11"].Title = "Selection 10 - The calculated invariant mass was within " + str(NumericalCuts.InvariantMassVariance) + " MeV of the true Delta1 Baryon mass, " + str(PhysicalConstants.Delta1BaryonMass) + " MeV - Events Remaining"
			This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents11"].Quantity = This.SelectionDictionary["DeltaPGamma"][11].EventsRemaining
			...
			This.EfficiencyStatistics_Delta1ToProtonPhotonPP.Statistics["NEvents9"].Title = "Selection 9 - The time separation was less than " + str(NumericalCuts.TimeSeparation) + " ms - Events Remaining"
		...
		THStack1 = This.HistogramCollection1.Histograms["TrackMultiplicity"].ToROOTHistogram()
		THStack2 = This.HistogramCollection1.Histograms["TorrentMultiplicity"].ToROOTHistogram()
		"""
		TCanvas1 = This.HistogramCollection1.Histograms["TrackMultiplicity"].ToROOTHistogram()
		...
		HistogramStorage2 = ROOTFile.HistogramStorage()
		
		HistogramStorage1.Histogram = THStack1
		HistogramStorage2.Histogram = THStack2
		
		This.ROOTFile1.HistogramDictionary["TrackMultiplicity"] = HistogramStorage1
		This.ROOTFile1.HistogramDictionary["TorrentMultiplicity"] = HistogramStorage2
		"""
		This.ROOTFile1.NewStackedHistogramCanvas("Canvas of Track Multiplicity", TCanvas1, "")
		This.ROOTFile1.NewStackedHistogramCanvas("Canvas of Torrent Multiplicity", TCanvas2, "")
		"""
	...
	def runEvent(This):
		...
		if (MuonProtonMeet and DeltaPGammaCriteria[7]):
		...
		if (This.SelectBackgroundEvents == True):
			if (DeltaPGammaCriteria[len(DeltaPGammaCriteria) - 1] == True):
				...
				if (TrueEventContainsDelta1ToProtonPhoton):
					This.DeltaEventNumbers.append(str(This.BasicInformation.RunID) + " " + str(This.BasicInformation.SubrunID) + " " + str(This.BasicInformation.EventID))

				if (Delta1BaryonToProtonPi0MesonInteraction == True):
					This.BackgroundPiMesonEventNumbers.append(str(This.BasicInformation.RunID) + " " + str(This.BasicInformation.SubrunID) + " " + str(This.BasicInformation.EventID))
					
				if (TruthProtonTrackInteractionType == "ES"):
					This.BackgroundElasticEventNumbers.append(str(This.BasicInformation.RunID) + " " + str(This.BasicInformation.SubrunID) + " " + str(This.BasicInformation.EventID))
					
				if (TruthProtonTrackInteractionType == "DIS"):
					This.BackgroundDISEventNumbers.append(str(This.BasicInformation.RunID) + " " + str(This.BasicInformation.SubrunID) + " " + str(This.BasicInformation.EventID))
					
				if ((TruthProtonTrackInteractionType == "RP") and (not TrueEventContainsDelta1ToProtonPhoton) and (not TrueEventContainsDelta1ToProtonPi0)):
					This.BackgroundOtherResonanceEventNumbers.append(str(This.BasicInformation.RunID) + " " + str(This.BasicInformation.SubrunID) + " " + str(This.BasicInformation.EventID))

				if (TruthProtonTrackInteractionType == "C"):
					This.BackgroundCoherentEventNumbers.append(str(This.BasicInformation.RunID) + " " + str(This.BasicInformation.SubrunID) + " " + str(This.BasicInformation.EventID))
					
				if (TruthProtonTrackInteractionType == "QS"):
					This.BackgroundQESEventNumbers.append(str(This.BasicInformation.RunID) + " " + str(This.BasicInformation.SubrunID) + " " + str(This.BasicInformation.EventID))
	...
	def MuonProtonSameVertex(This, Muon, Proton):
		...
		"""
		MuonFrontPosition = Geometry.ThreeVector(Muon.FrontPosition.X, Muon.FrontPosition.Y, Muon.FrontPosition.Z)
		ProtonFrontPosition = Geometry.ThreeVector()
		
		ProtonFrontPositionFourVector = Proton.Position
		
		ProtonFrontPosition.X = ProtonFrontPositionFourVector.X#Pehaps there could be a Four Vector -> Three Vector converter
		ProtonFrontPosition.Y = ProtonFrontPositionFourVector.Y
		ProtonFrontPosition.Z = ProtonFrontPositionFourVector.Z
		"""
