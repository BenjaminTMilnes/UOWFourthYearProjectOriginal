...
class Analysis:
		...
		for i in range(EventNumber):
			...
			if (This.InputTimeTest == False):
				...
				if (SuitableEvent):
					...
					CurrentTime = ElapsedTime(StartTime, 15000000000000000000000000000)#One Hour
				...
				if ((OutputData or (i == EventNumber - 1)) and (SuitableEvents > 0)):
					...
					for Key, SelectionChannel in This.SelectionDictionary.iteritems():
						...
						for CriteriaListCounter in xrange(CutNumber):
							...
							if(SelectionChannel[CriteriaListCounter].MixedPurity > 0):
								...
								This.StackedHistograms[NormalisedPurityReference].AttemptToFill2D(NormalisedPurityTitle, "Mixed Events", "NormalisedPurity:Mixed" + Key, "Selections", "Purity", CutNumber, 0, CutNumber, Location + "/Normalised/Constituent Histograms/", CriteriaListCounter, SelectionChannel[CriteriaListCounter].MixedPurity, "Mixed Events")
					...
					############## Application of truth to criteria
						...
						for CutCounter in xrange(len(SelectionChannel)):
							...
							for FillCounter in xrange(SelectionChannel[CutCounter].MixedEvent):
								This.StackedHistograms[CutPurityReference].AttemptToFill1D(NormalisedPurityTitle, "Mixed Events", "Purity:Mixed" + Key, "Selections", "Purity", CutNumber, 0, CutNumber, Location + "/Energy/Constituent Histograms/", CutCounter, "Mixed Events")
					...
					This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents7"].Title = "Selection 7 - Fewer than " + str(This.Parameters["PathMultiplicityLimit"]) + " PIDs in total - Events Remaining"
					...
					This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents9"].Title = "Selection 9 - The smallest angle was less than " + str(This.Parameters["AngularLocalityLimit"]) + " degees - Events Remaining"
					...
					This.EfficiencyStatistics_Delta1ToProtonPhoton.Statistics["NEvents10"].Title = "Selection 10 - The calculated invariant mass was within " + str(This.Parameters["InvariantMassVarianceLimit"]) + " MeV of the true Delta1 Baryon mass, " + str(PhysicalConstants.Delta1BaryonMass) + " MeV - Events Remaining"
					...
					This.EfficiencyStatistics_Delta1ToProtonPi0.Statistics["NEvents7"].Title = "Selection 7 - Fewer than " + str(This.Parameters["PathMultiplicityLimit"]) + " PIDs in total - Events Remaining"
					...
					This.EfficiencyStatistics_Delta1ToProtonPhotonPP.Statistics["NEvents9"].Title = "Selection 9 - The smallest angle was less than " + str(This.Parameters["AngularLocalityLimit"]) + " degees - Events Remaining"
					...
					#sys.stdout = DataWriter.DataWriter(This.SimulationInformationFileLocator, "a")#return back to simulation information file
	...
	def runEvent(This):
			...
			for Vertex1 in This.Truth_Vertices.Vertices: #Then Loop over all vertices
				...
				for DeltaVertexID in DeltaVertexList:#Looping over Delta Baryon vertex IDs
					...
					if(Vertex1.ID == DeltaVertexID):#Selects the Delta Baryon vertices
						...
						for TrajectoryID in Vertex1.PrimaryTrajIDs:#Each Delta Baryon Vertex has a list of daughter trajectories
							
							if (TrajectoryID == Trajectory1.ID):#Selects the trajectories that are part of Delta Baryon daughter trajectories
					...
					for DeltaPhotonTrajectoryID in DeltaToPhotonTrajectoryIDs:
						...
						if(DeltaPhotonTrajectoryID == ElectronParent):#Finds if the electron positron pairs are from a 
							...
							for Trajectory1 in This.Truth_Trajectories.Trajectories:
								...
								if (Trajectory1.ID == ElectronParent):
		...
		if (SmallestAngle <= This.Parameters["AngularLocalityLimit"] and DeltaPGammaCriteria[9]):
