...
class Analysis:
	...
	def Analyse(self, n = 999999999):
		...
		self.ROOTFile1.NewHistogram1D("Reconstructed_Delta1Baryon_Energy", "Energy of the Unknown Particle which created the Reconstructed Proton and Photon", "Energy (GeV)", "Number", 100, 0, 5000, "DeltaBaryon/Recon/")#!!! Filename system is not currently working
		...
		if (self.InputTimeTest == False):
			...
			self.ROOTFile1.NewHistogram1D("Absolute_Efficiency", "Absolute Efficiency for the series of cuts", "Cuts", "Absolute Efficiency", CutNumber, 0, CutNumber, "Cuts/")

			self.ROOTFile1.HistogramDictionary["Absolute_Efficiency"].Histogram.SetMarkerStyle(3)
			self.ROOTFile1.HistogramDictionary["Absolute_Efficiency"].Histogram.SetOption("LP")
			self.ROOTFile1.HistogramDictionary["Absolute_Efficiency"].Histogram.SetStats(0)

			for CriteriaListCounter in xrange(CutNumber):
				
				self.ROOTFile1.HistogramDictionary["Absolute_Efficiency"].Histogram.Fill(CriteriaListCounter,self.SelectionCriteriaList[CriteriaListCounter].AbsoluteEfficiency)
			...
			for CutCounter in xrange(len(self.SelectionCriteriaList)):
				...
				self.ROOTFile1.HistogramDictionary["Absolute_Efficiency"].Histogram.GetXaxis().SetBinLabel(CutCounter+1, BinLabel)

			self.ROOTFile1.NewHistogram1D("Relative_Efficiency", "Relative Efficiency for the series of cuts", "Cuts", "Relative Efficiency", CutNumber, 0, CutNumber, "Cuts/")

			self.ROOTFile1.HistogramDictionary["Relative_Efficiency"].Histogram.SetMarkerStyle(3)
			self.ROOTFile1.HistogramDictionary["Relative_Efficiency"].Histogram.SetOption("LP")
			self.ROOTFile1.HistogramDictionary["Relative_Efficiency"].Histogram.SetStats(0)

			for CriteriaListCounter in xrange(CutNumber):
				
				self.ROOTFile1.HistogramDictionary["Relative_Efficiency"].Histogram.Fill(CriteriaListCounter,self.SelectionCriteriaList[CriteriaListCounter].RelativeEfficiency)
			...
			for CutCounter in xrange(len(self.SelectionCriteriaList)):
				...
				self.ROOTFile1.HistogramDictionary["Relative_Efficiency"].Histogram.GetXaxis().SetBinLabel(CutCounter+1, BinLabel)
	
			self.ROOTFile1.NewHistogram1D("Purity", "Purity for the series of cuts", "Cuts", "Purity", CutNumber, 0, CutNumber, "Cuts/")
			...
			for CriteriaListCounter in xrange(CutNumber):
				...
				self.ROOTFile1.HistogramDictionary["Purity"].Histogram.Fill(CriteriaListCounter,self.SelectionCriteriaList[CriteriaListCounter].Purity)
			...
			for CutCounter in xrange(len(self.SelectionCriteriaList)):
				...
				self.CutPurity.StackedHistogram.GetXaxis().SetBinLabel(CutCounter+1, BinLabel)
			
			self.CutPurity.DrawConstituentHistograms(self.ROOTFile1)
			
			for Histogram1Key in self.CutPurity.HistogramDictionary.keys():
				for CutCounter in xrange(len(self.SelectionCriteriaList)):
					...
					self.ROOTFile1.HistogramDictionary[Histogram1Key].Histogram.GetXaxis().SetBinLabel(CutCounter+1, BinLabel)
					
			###############

			print TextConstants.NewLine , TextConstants.LineSeparator , TextConstants.NewLine
			print "Cuts"
			print TextConstants.LineSeparator
			...
			print "Sixth Cut: Of the fifth cut, the events where the single proton track began in the FGD"
			print "Events remaining:" , str(self.SelectionCriteriaList[6].EventsRemaining)
			
			print "Seventh: Of the sixth cut, the events where at least one ECal cluster was recorded"
			print "Events remaining:" , str(self.SelectionCriteriaList[7].EventsRemaining)
		
			print TextConstants.LineSeparator, TextConstants.NewLine, TextConstants.NewLine, TextConstants.NewLine, TextConstants.NewLine
	...
	def runEvent(self):
		...
		for GRTVertex1 in self.Truth_GRTVertices.Vtx: #Loop over vertices in event
			...
			Delta1BaryonToProtonPi0MesonInteraction = (IncidentMuonNeutrino and ProtonFromDelta and Pi0MesonFromDelta and InteractionType == "RES")
		...
		for CutCounter in xrange(CutNumber):
			if (Criteria[CutCounter]):
				...
				elif (InteractionType == "RES" and not DeltaPGammaEvent and not Delta1BaryonToProtonPi0MesonEvent):
		...
		if (len(PhotonList) > 0 and len(ProtonList) > 0):
			...
			for Photon1 in PhotonList:
				for Proton1 in ProtonList:
					...
					print Angle1""" #This was causing a problem, so I just commented it out for now... I think it is still being worked on
