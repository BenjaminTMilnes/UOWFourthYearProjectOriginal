...
class ReconstructedParticle:
	...
	def __init__(This):
		...
		This.TrueGRTVertex = GRooTrackerVertex.GRooTrackerVertex()
	...
	def GetProcess(This, RTTools):
		...
		This.EventCodeDeconstructor1.ReadCode(This.TrueGRTVertex.EvtCode)
		...
		for GRTParticle1 in RTTools.getParticles(This.TrueGRTVertex):
			...
			if (GRTParticle1.pdg == ParticleCodes.Delta1Baryon):
				...
				if (DeltaDaughterNumber == 2):#Delta -> p gamma must have 2 daughters
					...
					for DaughterParticle in RTTools.getParticles(This.TrueGRTVertex):#Loop again over particles in vertex
		...
		This.Process = EventProcess(ContainsDelta1ToProtonPhoton, ContainsDelta1ToProtonPi0, This.InteractionType)
...
class ReconstructedObject:
	...
	def CorrectlyReconstructed(This):
		#Note: Currently only works for electrons
		CorrectlyReconstructed = False
		
		if (This.Charge == - 1):
			
			if (This.ReconParticleID == This.TrueParticleID):
				
				CorrectlyReconstructed = True
